...
В `src/map_capture_profiles.c` расширить board-профиль: **4 approved-вектора
на строку** (центр + 3 смещения), id-пространство 48 вариантов:

```text
id = 0x424F4152 + row*4 + point,  row = sector*2 + window (0..11), point 0..3
```

Вектора для сектора s (центр (cu,cv,cw) из MAP_CAPTURE_BOARD_MOD, CCR
375/500/625):

| point | смещение Q15 | CCR (для центра 625,500,375) |
|---|---|---|
| 0 | (0,0,0) | (625,500,375) |
| 1 | (+262,0,−262) | (629,500,371) |
| 2 | (0,+262,−262) | (625,504,371) |
| 3 | (−262,+262,0) | (621,504,375) |

(±4 CCR-тика = ±262 Q15; разброс по осям 262..524 ≥ 2·guard_q15 ✓;
порядок фаз сектора сохраняется ✓; все точки в апертуре 135..999 ✓;
в границах region ±2048 ✓. Пересчитать для каждого сектора по формуле
`ccr = 500 + mod*500/32768`.)

- Требования профиля: exact-match на полный запрос (как сейчас), sector/window
  из id, точка из id; `pulse_count` 16 (или 8 — решить в реализации);
  `min_records_per_row` ≥ 3; region/recon как в BOAR v1 (recon.valid=false,
  fail-closed до offline-оценки).
- Тесты: 48 вариантов дают корректные (sector, window, point, CCR);
  изменённый CCR/сектор → REJECT; production (без OEW_MAP_CAPTURE+L3) → deny.
