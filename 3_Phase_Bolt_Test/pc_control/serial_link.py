...
class MotorSerialLink:
    ...
    def __init__(self, port=None, baudrate=DEFAULT_BAUDRATE):
        ...
        # TestR info callback ($TESTR line with avg currents and raw ADC)
        self.testr_info = {}
        self.on_testr_info = None
    ...
    def _handle_line(self, text):
        ...
        elif text.startswith("$TESTR,"):
            self._parse_testr_info(text)
    ...
    def _parse_testr_info(self, text):
        """Parse $TESTR,ia_avg=...,ib_avg=...,raw_a1=...,hint_a1=...,... line."""
        try:
            info = {}
            parts = text.split(",")
            for p in parts[1:]:
                if "=" in p:
                    ...
                    try:
                        ...
                        info[k] = v
            self.testr_info = info
            if self.on_testr_info:
                try:
                    self.on_testr_info(info)
                except Exception:
                    pass
        except Exception:
            pass
