...
class MotorControlApp:
    ...
    def __init__(self, root):
        ...
        self.link.on_testr_info = self._on_testr_info
    ...
    def _build_ui(self):
        ...
        self._build_plots()
        self._build_terminal_log()

    def _build_terminal_log(self):
        frame = ttk.LabelFrame(self.root, text="MCU Terminal")
        frame.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True, padx=6, pady=4)
        self.term_log = scrolledtext.ScrolledText(
            frame, height=8, wrap=tk.WORD, state=tk.DISABLED, bg="black", fg="lime", insertbackground="white"
        )
        self.term_log.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)
        self.term_log.tag_config("warning", foreground="orange")
        self.term_log.tag_config("error", foreground="red")
        self.term_log.tag_config("info", foreground="cyan")
    ...
    def _build_control_panel(self):
        ...
        self.test_r_btn = ttk.Button(row4, text="Test R", command=self._cmd_test_r)
        self.test_r_btn.pack(side=tk.LEFT, padx=4)
    ...
    def _on_text_line(self, text):
        try:
            self.root.after(0, lambda: self._append_terminal(text))
        except Exception:
            pass
    ...
    def _append_terminal(self, text):
        if not hasattr(self, "term_log"):
            return
        self.term_log.configure(state=tk.NORMAL)
        tag = ""
        if "ERROR" in text or "FAULT" in text or "fault" in text.lower():
            tag = "error"
        elif "WARNING" in text or "WARN" in text:
            tag = "warning"
        elif text.startswith("$") or text.startswith("R_TEST_DONE") or text.startswith("L_TEST"):
            tag = "info"
        self.term_log.insert(tk.END, text + "\n", tag)
        # Keep last 200 lines
        lines = int(self.term_log.index(tk.END).split(".")[0])
        if lines > 200:
            self.term_log.delete("1.0", "%d.0" % (lines - 200))
        self.term_log.see(tk.END)
        self.term_log.configure(state=tk.DISABLED)
    ...
    def _cmd_test_r(self):
        if not self.link.is_open: return messagebox.showerror("Error", "Not connected")
        if not messagebox.askyesno("Test R Warning", "Убедитесь, что ротор ЗАФИКСИРОВАН и фиксируйте ток фазы A мультиметром!\nПродолжить?"): return
        self.cap_status.config(text="Testing R (3s DC)...", fg="orange")
        self.link.test_resistance()
    ...
    def _on_testr_info(self, info):
        """Called from reader thread when $TESTR line is received.
        Marshal into main thread for safe Tk access."""
        try:
            self.root.after(0, lambda: self._show_testr_info(info))
        except Exception:
            pass

    def _show_testr_info(self, info):
        """Display testR average currents, raw ADC and scale hints."""
        try:
            ia = float(info.get("ia_avg", 0.0))
            ib = float(info.get("ib_avg", 0.0))
            ia2 = float(info.get("ia2_avg", 0.0))
            ib2 = float(info.get("ib2_avg", 0.0))
            ra1 = float(info.get("raw_a1", 0.0))
            rb1 = float(info.get("raw_b1", 0.0))
            ra2 = float(info.get("raw_a2", 0.0))
            rb2 = float(info.get("raw_b2", 0.0))
            hint_a1 = float(info.get("hint_a1", 0.0))
            hint_b1 = float(info.get("hint_b1", 0.0))
            cal1 = int(info.get("cal1", 0))
            cal2 = int(info.get("cal2", 0))
            n = int(info.get("samples", 0))
            txt = ("Test R done: ia=%.3f A  ib=%.3f A  ia2=%.3f A  ib2=%.3f A  "
                   "rA1=%.0f rB1=%.0f rA2=%.0f rB2=%.0f  hint_A=%.6f A/LSB  cal1=%d cal2=%d n=%d") % (
                   ia, ib, ia2, ib2, ra1, rb1, ra2, rb2, hint_a1, cal1, cal2, n)
            self.cap_status.config(text=txt, fg="blue")
        except Exception as e:
            self.cap_status.config(text="Test R parse err: %s" % e, fg="red")
