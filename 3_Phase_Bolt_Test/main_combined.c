/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Combined OEW Motor Control (FOC + V/f) with Hardware Safety
  *
  * Скрещенная версия: FOC-алгоритм из Проекта 1 + аппаратная безопасность
  * из Проекта 2. См. сводку изменений в комментарии ниже.
  *
  * Ключевые особенности:
  *   - Три режима: MODE_COMP, MODE_OEW_SPLIT_SVPWM, MODE_FOC_OEW
  *   - Полный FOC: Clarke -> Park -> PI(Id, Iq, omega) -> Inv-Park -> Inv-Clarke
  *   - Наблюдатель потокосцепления ротора (RotorFluxObserver)
  *   - Корректный обратный Кларк (амплитудно-инвариантный)
  *   - ZSC PI с relay-автонастройкой (Зиглер-Никольс)
  *   - Аппаратный Break Input на TIM1/TIM8 (LOW polarity) - критично для power stage
  *   - DeadTime=255 (~2.5 мкс при 170 МГц)
  *   - AutoReloadPreload=ENABLE (защита от ARR/CCR glitch)
  *   - PWM_PERIOD=17000 (HALF_PERIOD=8500 - чистое деление)
  *   - TIM8 в SLAVEMODE_RESET (надёжнее TRIGGER для production)
  *   - Injected ADC dual-mode INJECSIMULT (синхронно ADC1+ADC2 для FOC)
  *   - HAL_ADCEx_Calibration_Start при старте + рантайм-калибровка offset
  *   - Динамический dt: измерение ISR_Hz каждую секунду, обновление control_isr_dt
  *   - LED-индикатор: горит при ISR_Hz в допуске, мигает при отклонении
  *   - Опционально: field weakening, Rs drop comp, IWDG watchdog
  *   - Burst capture (512 семплов), testL для измерения индуктивности
  *   - Auto-start: calib -> spin -> ZSC autotune -> restore targets
  *   - JSON telemetry stream
  ******************************************************************************
  */
/* USER CODE END Header */

/* Includes ------------------------------------------------------------------*/
#include "main.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
/* USER CODE END Includes */

/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
typedef enum {
    MODE_COMP = 0,                  /* TIM8 = инверсная копия TIM1 (чистая синусоида) */
    MODE_OEW_SPLIT_SVPWM = 1,       /* Разделенное OEW управление с ZSC PI */
    MODE_FOC_OEW = 2                /* Полный FOC с OEW и ZSC */
} OewModulationMode_t;

typedef enum {
    ZSC_ATUNE_IDLE    = 0,
    ZSC_ATUNE_RUNNING = 1,
    ZSC_ATUNE_DONE    = 2,
    ZSC_ATUNE_FAILED  = 3
} ZscAtuneState_t;

typedef struct {
    ZscAtuneState_t state;
    float relay_amp, izs_prev, peak_pos, peak_neg, half_period_sum;
    uint32_t half_periods, ticks, ticks_since_cross;
    float result_kp, result_ki;
} ZscAutotune_t;

typedef struct {
    float ia, ib, ic;
    float ia_raw, ib_raw, ic_raw;
    float off_a, off_b, off_c;
    float scale_a, scale_b, scale_c;
    uint8_t calibrated;
    uint32_t calib_cnt;
    float rms2_lpf;
} CurrentMeas_t;

typedef struct {
    float kp, ki, integ, out_min, out_max;
} PI_t;

/* --- FOC Math Definitions --- */
typedef struct {
    float Rs, Rr, Ls, Lr, Lm, p_pairs;
} MotorParams_t;

typedef struct {
    float psi_alpha, psi_beta, theta, omega_r;
    float id, iq, vd, vq;
} FOC_State_t;

/* --- Auto-Start State Machine --- */
typedef enum {
    AUTO_START_IDLE   = 0,
    AUTO_START_CALIB  = 1,
    AUTO_START_SPIN   = 2,
    AUTO_START_ZSC    = 3,
    AUTO_START_DONE   = 4
} AutoStartState_t;

typedef enum {
    FOC_STARTUP_IDLE    = 0,
    FOC_STARTUP_RAMPING = 1,
    FOC_STARTUP_RUNNING = 2
} FocStartupState_t;

/* --- Burst capture --- */
typedef struct {
    volatile uint8_t armed;
    volatile uint16_t count;
    volatile uint8_t busy;
    uint32_t isr_hz;
} BurstCtrl_t;

typedef struct {
    int16_t ia1, ib1, ic1;
    int16_t ia2, ib2, ic2;
    int16_t izs;
    uint16_t ang;
    uint16_t d1a, d1b, d1c;
    uint16_t d2a, d2b, d2c;
} BurstSample_t;
/* USER CODE END PTD */

/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define PWM_PERIOD          17000           /* было 16999: теперь HALF_PERIOD=8500 - чисто */
#define HALF_PERIOD         (PWM_PERIOD / 2)

#ifndef M_PI
#define M_PI                3.14159265358979323846f
#endif
#define TWO_PI              6.28318530718f
#define SQRT3               1.7320508f
#define SQRT3_OVER_2        0.8660254f
#define ONE_OVER_SQRT3      0.57735026919f

#define SPEED_RATE          100.0f
#define POWER_RATE          2.0f
#define MAX_POWER           0.98f
#define MAX_SPEED_ELEC_HZ   400.0f

#define CORDIC_TIMEOUT_CYCLES   1000U
#define EXPECTED_ISR_HZ         5000.0f
#define ISR_HZ_TOLERANCE        0.05f
#define CURR_IIR_ALPHA          0.2f
#define CURR_CALIB_SAMPLES      2000U

#define I_DIDT_MAX_PER_TICK_DEFAULT  1.5f
#define I_PHASE_ABS_MAX_A_DEFAULT    8.0f
#define I_RMS_MAX_A_DEFAULT          5.0f
#define I_RMS_LPF_ALPHA              0.01f

#define PI_KP_DEFAULT   0.08f
#define PI_KI_DEFAULT   20.0f

#define ZSC_KP          0.05f
#define ZSC_KI          0.5f
#define ZSC_OUT_MAX     ((float)HALF_PERIOD * 0.15f)

#define ZSC_ATUNE_RELAY_AMP    (ZSC_OUT_MAX * 0.8f)
#define ZSC_ATUNE_MIN_PERIODS  6U
#define ZSC_ATUNE_MAX_TICKS    50000U

#define AUTO_CALIB_TIMEOUT_MS  1000U
#define AUTO_SPIN_TIME_MS      2000U
#define AUTO_ZSC_TIMEOUT_MS    5000U

#define FOC_MIN_STARTUP_HZ     15.0f
#define FOC_LOST_HZ            5.0f

/* --- Опциональные расширенные функции (включать по мере необходимости) --- */
#define FIELD_WEAKENING_ENABLE   0   /* 1 = ослабление поля при omega > omega_base */
#define RS_DROP_COMP_ENABLE      0   /* 1 = компенсация падения на Rs (low-speed boost) */
#define IWDG_WATCHDOG_ENABLE     0   /* 1 = независимый watchdog */

#if FIELD_WEAKENING_ENABLE
#define FW_BASE_SPEED_HZ         150.0f   /* частота, с которой начинается ослабление поля */
#define FW_ID_NOMINAL            1.0f     /* номинальный Id_target в о.е. */
#endif

/* --- Аппаратная защита силового каскада (взято из Проекта 2) --- */
#define PWM_DEADTIME             255      /* ~2.5 мкс при 170 МГц */
#define PWM_BREAK_POLARITY_LOW   1        /* 1 = активный ноль (стандарт для STP/IGBT) */
/* USER CODE END PD */

/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */

/* USER CODE END PM */

/* Private variables ---------------------------------------------------------*/
__IO uint32_t BspButtonState = BUTTON_RELEASED;
ADC_HandleTypeDef hadc1;
ADC_HandleTypeDef hadc2;

CORDIC_HandleTypeDef hcordic;

TIM_HandleTypeDef htim1;
TIM_HandleTypeDef htim8;

UART_HandleTypeDef huart2;

/* USER CODE BEGIN PV */
volatile float Target_Speed = 50.0f;
volatile float Target_Power = 0.5f;
float current_speed = 0.0f;
float current_power = 0.0f;
float ugol1 = 0.0f;
volatile uint32_t isr_tick_count = 0U;
volatile float measured_isr_hz = EXPECTED_ISR_HZ;
volatile float control_isr_dt = (1.0f / EXPECTED_ISR_HZ);
volatile OewModulationMode_t oew_mode = MODE_OEW_SPLIT_SVPWM;
volatile float Target_Current_A = 2.0f;
volatile uint8_t Fault_OverCurrent = 0U;
volatile uint8_t Run_Enabled = 0U;

CurrentMeas_t g_curr  = { .scale_a = 0.012788f, .scale_b = 0.012788f, .scale_c = 0.012788f };
CurrentMeas_t g_curr2 = { .scale_a = 0.012788f, .scale_b = 0.012788f, .scale_c = 0.012788f };

PI_t g_i_pi     = { .kp = PI_KP_DEFAULT, .ki = PI_KI_DEFAULT, .integ = 0.0f, .out_min = 0.0f, .out_max = MAX_POWER };
PI_t g_zsc_pi   = { .kp = ZSC_KP, .ki = ZSC_KI, .integ = 0.0f, .out_min = -ZSC_OUT_MAX, .out_max = ZSC_OUT_MAX };
ZscAutotune_t g_zsc_atune = { .state = ZSC_ATUNE_IDLE };

PI_t g_pi_id    = { .kp = 0.5f, .ki = 500.0f, .out_min = -1.0f, .out_max = 1.0f };
PI_t g_pi_iq    = { .kp = 0.5f, .ki = 500.0f, .out_min = -1.0f, .out_max = 1.0f };
PI_t g_pi_speed = { .kp = 0.5f, .ki = 10.0f, .out_min = -5.0f, .out_max = 5.0f };

MotorParams_t g_motor = { .Rs = 0.5f, .Rr = 0.3f, .Ls = 0.001f, .Lr = 0.001f, .Lm = 0.0009f, .p_pairs = 2.0f };
FOC_State_t g_foc = {0};
volatile FocStartupState_t foc_startup_state = FOC_STARTUP_IDLE;

volatile AutoStartState_t auto_start_state = AUTO_START_IDLE;
static float autosave_speed = 0;
static float autosave_power = 0;
static float autosave_current = 0;
static OewModulationMode_t autosave_mode = MODE_OEW_SPLIT_SVPWM;
static uint32_t auto_start_t0 = 0U;

static float i_prev_a = 0.0f, i_prev_b = 0.0f, i_prev_c = 0.0f;

volatile uint8_t Stream_Enabled = 0;
volatile float izs_val = 0.0f;

static volatile uint16_t last_raw_a1 = 0U, last_raw_b1 = 0U, last_raw_c1 = 0U;
static volatile uint16_t last_raw_a2 = 0U, last_raw_b2 = 0U, last_raw_c2 = 0U;

#define BURST_CAP_SIZE    512U
#define BURST_DUMP_CHUNK  16
static BurstSample_t burst_buf[BURST_CAP_SIZE];
static BurstCtrl_t   burst = {0};

#define UART_RX_BUF_SIZE 64U
static uint8_t uart_rx_byte = 0U;
static volatile char uart_rx_buf[UART_RX_BUF_SIZE];
static volatile uint16_t uart_rx_len = 0U;
static volatile uint8_t  uart_rx_ready = 0U;

/* Флаги для безопасной печати из главного цикла (вместо printf в ISR) */
volatile uint8_t print_foc_engaged = 0U;
volatile uint8_t print_foc_lost = 0U;
volatile uint8_t print_testl = 0U;

/* Перетаскиваемые лимиты тока (можно менять из UART) */
float I_PHASE_ABS_MAX_A = I_PHASE_ABS_MAX_A_DEFAULT;
float I_RMS_MAX_A = I_RMS_MAX_A_DEFAULT;
float I_DIDT_MAX_PER_TICK = I_DIDT_MAX_PER_TICK_DEFAULT;

#define TEST_L_SAMPLES 50U
static volatile uint8_t  test_l_active = 0U;
static volatile uint16_t test_l_idx = 0U;
static volatile int16_t  test_l_ia[TEST_L_SAMPLES];
/* USER CODE END PV */

/* Private function prototypes -----------------------------------------------*/
void SystemClock_Config(void);
static void MX_GPIO_Init(void);
static void MX_ADC1_Init(void);
static void MX_ADC2_Init(void);
static void MX_TIM1_Init(void);
static void MX_TIM8_Init(void);
static void MX_CORDIC_Init(void);
static void MX_USART2_UART_Init(void);

/* USER CODE BEGIN PFP */
static void uart2_tx_blocking(const uint8_t *p, uint16_t n);
static inline float clampf(float v, float min, float max) { return (v < min) ? min : ((v > max) ? max : v); }

static inline uint32_t clamp_u32_from_float(float v, uint32_t min, uint32_t max) {
    if (!isfinite(v)) return min;
    if (v < (float)min) return min;
    if (v > (float)max) return max;
    return (uint32_t)v;
}

static inline float pi_step(PI_t *pi, float err, float dt) {
    float p = pi->kp * err;
    pi->integ += pi->ki * err * dt;
    float out = p + pi->integ;
    if (out > pi->out_max) { out = pi->out_max; pi->integ = out - p; }   /* anti-windup back-calc */
    else if (out < pi->out_min) { out = pi->out_min; pi->integ = out - p; }
    return out;
}

static void current_update(CurrentMeas_t *c, float a_adc, float b_adc, float c_adc) {
    if (!c->calibrated) {
        c->off_a += a_adc; c->off_b += b_adc; c->off_c += c_adc; c->calib_cnt++;
        if (c->calib_cnt >= CURR_CALIB_SAMPLES) {
            float k = 1.0f / (float)c->calib_cnt;
            c->off_a *= k; c->off_b *= k; c->off_c *= k; c->calibrated = 1U;
        }
        c->ia = 0.0f; c->ib = 0.0f; c->ic = 0.0f;
        return;
    }
    c->ia_raw = (a_adc - c->off_a) * c->scale_a;
    c->ib_raw = (b_adc - c->off_b) * c->scale_b;
    c->ic_raw = (c_adc - c->off_c) * c->scale_c;
    c->ia += CURR_IIR_ALPHA * (c->ia_raw - c->ia);
    c->ib += CURR_IIR_ALPHA * (c->ib_raw - c->ib);
    c->ic += CURR_IIR_ALPHA * (c->ic_raw - c->ic);
    c->rms2_lpf += I_RMS_LPF_ALPHA * ((c->ia * c->ia + c->ib * c->ib + c->ic * c->ic) * 0.333333f - c->rms2_lpf);
}

static void pwm_shutdown_fault(void) {
    TIM1->BDTR &= ~TIM_BDTR_MOE;
    TIM8->BDTR &= ~TIM_BDTR_MOE;
    Fault_OverCurrent = 1U;
}

static float zsc_autotune_step(ZscAutotune_t *at, float izs, float dt) {
    at->ticks++; at->ticks_since_cross++;
    if (at->ticks > ZSC_ATUNE_MAX_TICKS) { at->state = ZSC_ATUNE_FAILED; return 0.0f; }
    float vzs_corr = (izs >= 0.0f) ? at->relay_amp : -at->relay_amp;
    int cross = ((at->izs_prev >= 0.0f) && (izs < 0.0f)) || ((at->izs_prev < 0.0f) && (izs >= 0.0f));
    if (cross && (at->ticks_since_cross > 5U)) {
        at->half_period_sum += (float)at->ticks_since_cross * dt;
        at->half_periods++; at->ticks_since_cross = 0U;
        float amp = fmaxf(at->peak_pos, -at->peak_neg);
        if (amp < 1e-6f) amp = 1e-6f;
        if (at->half_periods >= ZSC_ATUNE_MIN_PERIODS) {
            float Tu = 2.0f * (at->half_period_sum / (float)at->half_periods);
            float Ku = (4.0f * at->relay_amp) / (3.14159265f * amp);
            at->result_kp = 0.45f * Ku; at->result_ki = 0.54f * Ku / Tu;
            g_zsc_pi.kp = at->result_kp; g_zsc_pi.ki = at->result_ki; g_zsc_pi.integ = 0.0f;
            at->state = ZSC_ATUNE_DONE; return 0.0f;
        }
        at->peak_pos = 0.0f; at->peak_neg = 0.0f;
    }
    if (izs > at->peak_pos) at->peak_pos = izs;
    if (izs < at->peak_neg) at->peak_neg = izs;
    at->izs_prev = izs;
    return vzs_corr;
}

/* CORDIC: прямой доступ к регистрам + явная конфигурация CSR.
 * Используем MODIFY_REG, чтобы не сбросить прочие поля.
 * FUNC=0 (Cosine), PRECISION=24 (высокая точность для FOC). */
static float CORDIC_Calc(float angle_rad, float *last_result_ptr) {
    angle_rad = fmodf(angle_rad + M_PI, TWO_PI);
    if (angle_rad < 0) angle_rad += TWO_PI;
    angle_rad -= M_PI;
    int32_t angle_q31 = (int32_t)(angle_rad * (2147483648.0f / M_PI));
    CORDIC->WDATA = angle_q31;
    uint32_t timeout = CORDIC_TIMEOUT_CYCLES;
    while (((CORDIC->CSR & CORDIC_CSR_RRDY) == 0U) && (timeout-- > 0U)) { }
    if ((CORDIC->CSR & CORDIC_CSR_RRDY) == 0U) return *last_result_ptr;
    int32_t result_q31 = CORDIC->RDATA;
    *last_result_ptr = (float)result_q31 / 2147483648.0f;
    return *last_result_ptr;
}

static float CORDIC_Cos(float angle_rad) {
    static float last_cos = 1.0f;
    return CORDIC_Calc(angle_rad, &last_cos);
}

static float CORDIC_Sin(float angle_rad) {
    static float last_sin = 0.0f;
    return CORDIC_Calc(angle_rad - 1.5707963f, &last_sin);
}

/* Преобразования Кларка/Парка. Прямой Кларк - амплитудно-инвариантный через ia,ib (ic=-ia-ib). */
static void Clarke_Transform(float ia, float ib, float *ialpha, float *ibeta) {
    *ialpha = ia;
    *ibeta  = (1.0f / SQRT3) * ia + (2.0f / SQRT3) * ib;
}

/* Обратный Кларк - ИСПРАВЛЕНО (раньше был copy-paste из прямого Кларка). */
static void Inv_Clarke_Transform(float valpha, float vbeta, float *va, float *vb, float *vc) {
    *va =  valpha;
    *vb = -0.5f * valpha + SQRT3_OVER_2 * vbeta;
    *vc = -0.5f * valpha - SQRT3_OVER_2 * vbeta;
}

static void Park_Transform(float ialpha, float ibeta, float sin_t, float cos_t, float *id, float *iq) {
    *id =  ialpha * cos_t + ibeta * sin_t;
    *iq = -ialpha * sin_t + ibeta * cos_t;
}

static void Inv_Park_Transform(float vd, float vq, float sin_t, float cos_t, float *valpha, float *vbeta) {
    *valpha = vd * cos_t - vq * sin_t;
    *vbeta  = vd * sin_t + vq * cos_t;
}

/* Наблюдатель потокосцепления ротора (модель АД). Согласованный расчёт omega_e по старым psi. */
static void RotorFluxObserver(FOC_State_t *foc, MotorParams_t *m, float ialpha, float ibeta, float dt) {
    float Tr = m->Lr / m->Rr;
    float Lm_Tr = m->Lm / Tr;
    float inv_Tr = 1.0f / Tr;

    float psi_alpha_old = foc->psi_alpha;
    float psi_beta_old  = foc->psi_beta;

    float d_psi_alpha = (Lm_Tr * ialpha) - (inv_Tr * psi_alpha_old) + (foc->omega_r * psi_beta_old);
    float d_psi_beta  = (Lm_Tr * ibeta)  - (inv_Tr * psi_beta_old)  - (foc->omega_r * psi_alpha_old);

    float psi_mag_sq = psi_alpha_old * psi_alpha_old + psi_beta_old * psi_beta_old;
    if (psi_mag_sq < 1e-6f) psi_mag_sq = 1e-6f;

    float omega_e = (psi_alpha_old * d_psi_beta - psi_beta_old * d_psi_alpha) / psi_mag_sq;

    foc->psi_alpha = psi_alpha_old + d_psi_alpha * dt;
    foc->psi_beta  = psi_beta_old  + d_psi_beta  * dt;

    float omega_slip = (Lm_Tr * (psi_alpha_old * ibeta - psi_beta_old * ialpha)) / psi_mag_sq;
    foc->omega_r = omega_e - omega_slip;

    foc->theta = atan2f(foc->psi_beta, foc->psi_alpha);
    if (foc->theta < 0.0f) foc->theta += TWO_PI;
}

#if FIELD_WEAKENING_ENABLE
/* Ослабление поля: при omega > FW_BASE_SPEED_HZ линейно уменьшаем Id_target. */
static float field_weakening_id_target(float omega_r_hz, float id_nominal) {
    float abs_w = fabsf(omega_r_hz);
    if (abs_w <= FW_BASE_SPEED_HZ) return id_nominal;
    float overshoot = (abs_w - FW_BASE_SPEED_HZ) / (MAX_SPEED_ELEC_HZ - FW_BASE_SPEED_HZ);
    if (overshoot > 1.0f) overshoot = 1.0f;
    return id_nominal * (1.0f - overshoot);
}
#endif

void UART_Send_Stream(void) {
    static char buf[512];
    int ia1 = (int)(g_curr.ia * 1000),  ia1_i = ia1/1000,  ia1_f = ia1%1000;  if(ia1_f<0) ia1_f=-ia1_f;
    int ib1 = (int)(g_curr.ib * 1000),  ib1_i = ib1/1000,  ib1_f = ib1%1000;  if(ib1_f<0) ib1_f=-ib1_f;
    int ic1 = (int)(g_curr.ic * 1000),  ic1_i = ic1/1000,  ic1_f = ic1%1000;  if(ic1_f<0) ic1_f=-ic1_f;
    int ia2 = (int)(g_curr2.ia * 1000), ia2_i = ia2/1000,  ia2_f = ia2%1000;  if(ia2_f<0) ia2_f=-ia2_f;
    int ib2 = (int)(g_curr2.ib * 1000), ib2_i = ib2/1000,  ib2_f = ib2%1000;  if(ib2_f<0) ib2_f=-ib2_f;
    int ic2 = (int)(g_curr2.ic * 1000), ic2_i = ic2/1000,  ic2_f = ic2%1000;  if(ic2_f<0) ic2_f=-ic2_f;
    int izs = (int)(izs_val * 1000),    izs_i = izs/1000,  izs_f = izs%1000;  if(izs_f<0) izs_f=-izs_f;
    int irms= (int)(sqrtf(fmaxf(g_curr.rms2_lpf, 0.0f)) * 1000), irms_i=irms/1000, irms_f=irms%1000; if(irms_f<0) irms_f=-irms_f;
    int spd = (int)(current_speed * 100), spd_i = spd/100, spd_f = spd%100; if(spd_f<0) spd_f=-spd_f;
    int tgm = (int)(Target_Speed * 100), tgm_i = tgm/100, tgm_f = tgm%100; if(tgm_f<0) tgm_f=-tgm_f;
    int pwr = (int)(current_power * 1000), pwr_i = pwr/1000, pwr_f = pwr%1000; if(pwr_f<0) pwr_f=-pwr_f;
    int tgI = (int)(Target_Current_A * 1000), tgI_i = tgI/1000, tgI_f = tgI%1000; if(tgI_f<0) tgI_f=-tgI_f;
    int integ = (int)(g_zsc_pi.integ * 1000), integ_i = integ/1000, integ_f = integ%1000; if(integ_f<0) integ_f=-integ_f;

    int len = snprintf(buf, sizeof(buf),
        "{\"Ia1\":%d.%03d,\"Ib1\":%d.%03d,\"Ic1\":%d.%03d,\"Ia2\":%d.%03d,\"Ib2\":%d.%03d,\"Ic2\":%d.%03d,"
        "\"Izs\":%d.%03d,\"Irms\":%d.%03d,\"f\":%d.%02d,\"tgF\":%d.%02d,\"P\":%d.%03d,\"tgI\":%d.%03d,"
        "\"zint\":%d.%03d,\"mode\":%d,\"flt\":%d,\"run\":%d,\"foc\":%d,"
        "\"ticks\":%lu,\"cal\":%d,",
        ia1_i, ia1_f, ib1_i, ib1_f, ic1_i, ic1_f, ia2_i, ia2_f, ib2_i, ib2_f, ic2_i, ic2_f,
        izs_i, izs_f, irms_i, irms_f, spd_i, spd_f, tgm_i, tgm_f, pwr_i, pwr_f, tgI_i, tgI_f,
        integ_i, integ_f, (int)oew_mode, (int)Fault_OverCurrent, (int)Run_Enabled, (int)foc_startup_state,
        (unsigned long)isr_tick_count, (int)g_curr.calibrated);

    if (len > 0 && len < (int)sizeof(buf)) {
        int id_val = (int)(g_foc.id * 1000), id_i = id_val/1000, id_f = id_val%1000; if(id_f<0) id_f=-id_f;
        int iq_val = (int)(g_foc.iq * 1000), iq_i = iq_val/1000, iq_f = iq_val%1000; if(iq_f<0) iq_f=-iq_f;
        int vd_val = (int)(g_foc.vd * 1000), vd_i = vd_val/1000, vd_f = vd_val%1000; if(vd_f<0) vd_f=-vd_f;
        int vq_val = (int)(g_foc.vq * 1000), vq_i = vq_val/1000, vq_f = vq_val%1000; if(vq_f<0) vq_f=-vq_f;
        int wr_val = (int)(g_foc.omega_r * 100), wr_i = wr_val/100, wr_f = wr_val%100; if(wr_f<0) wr_f=-wr_f;
        int theta_val = (int)(g_foc.theta * 1000), th_i = theta_val/1000, th_th = theta_val%1000; if(th_th<0) th_th=-th_th;
        int isr_hz = (int)(measured_isr_hz * 10), ih_i = isr_hz/10, ih_f = isr_hz%10; if(ih_f<0) ih_f=-ih_f;

        snprintf(buf + len, sizeof(buf) - len,
            "\"Id\":%d.%03d,\"Iq\":%d.%03d,\"Vd\":%d.%03d,\"Vq\":%d.%03d,\"Wr\":%d.%02d,\"Th\":%d.%03d,"
            "\"ISR_Hz\":%d.%d}\r\n",
            id_i, id_f, iq_i, iq_f, vd_i, vd_f, vq_i, vq_f, wr_i, wr_f, th_i, th_th, ih_i, ih_f);
    } else {
        snprintf(buf, sizeof(buf), "{\"error\":\"buf_len\"}\r\n");
    }
    uart2_tx_blocking((uint8_t*)buf, (uint16_t)strlen(buf));
}

static void burst_dump_step(void) {
    static uint16_t idx = 0U;
    if (!burst.busy) { idx = 0U; return; }
    char line[128];
    uint16_t n = burst.count;
    uint16_t end = idx + BURST_DUMP_CHUNK;
    if (idx == 0U) {
        int len = snprintf(line, sizeof(line), "$BURST,%u,%lu\r\n", (unsigned)n, (unsigned long)burst.isr_hz);
        uart2_tx_blocking((uint8_t*)line, (uint16_t)len);
    }
    for (; (idx < end) && (idx < n); idx++) {
        const BurstSample_t *s = &burst_buf[idx];
        int len = snprintf(line, sizeof(line), "%u,%d,%d,%d,%d,%d,%d,%d,%u,%u,%u,%u,%u,%u,%u\r\n",
            (unsigned)idx, s->ia1, s->ib1, s->ic1, s->ia2, s->ib2, s->ic2, s->izs,
            s->ang, s->d1a, s->d1b, s->d1c, s->d2a, s->d2b, s->d2c);
        uart2_tx_blocking((uint8_t*)line, (uint16_t)len);
    }
    if (idx >= n) {
        uart2_tx_blocking((uint8_t*)"$END\r\n", 6U);
        burst.busy = 0U; idx = 0U;
    }
}

/* process_uart_command: объединённый набор команд из обоих проектов.
 * Добавлены: ?, ?i, m0/m1/m2, s, p, i, KP, KI, IM, IR, DT, MP, start, stop,
 *            stream on/off, faultclr, zscatune, capture, testL, calib */
void process_uart_command(const char *cmd) {
    if ((cmd == NULL) || (cmd[0] == '\0')) return;
    uint32_t primask;

    if (strcmp(cmd, "start") == 0) {
        autosave_speed   = Target_Speed;
        autosave_power   = Target_Power;
        autosave_current = Target_Current_A;
        autosave_mode    = oew_mode;

        primask = __get_PRIMASK(); __disable_irq();
        Target_Power = 0.0f; current_speed = 0.0f;
        Fault_OverCurrent = 0U;
        TIM1->BDTR |= TIM_BDTR_MOE; TIM8->BDTR |= TIM_BDTR_MOE;

        g_curr.calibrated = 0U;  g_curr.calib_cnt = 0U;
        g_curr.off_a = 0.0f; g_curr.off_b = 0.0f; g_curr.off_c = 0.0f;
        g_curr2.calibrated = 0U; g_curr2.calib_cnt = 0U;
        g_curr2.off_a = 0.0f; g_curr2.off_b = 0.0f; g_curr2.off_c = 0.0f;

        oew_mode = MODE_OEW_SPLIT_SVPWM;
        foc_startup_state = FOC_STARTUP_IDLE;
        Target_Speed    = 20.0f;
        Target_Power    = 0.5f;
        Target_Current_A = 2.0f;
        __set_PRIMASK(primask);

        auto_start_state = AUTO_START_CALIB;
        auto_start_t0 = HAL_GetTick();
        Run_Enabled = 1U;
        printf("AUTO START: Calibrating...\r\n");
    }
    else if (strcmp(cmd, "stop") == 0) {
        primask = __get_PRIMASK(); __disable_irq();
        Run_Enabled = 0U;
        Target_Power = 0.0f; current_speed = 0.0f; current_power = 0.0f;
        g_i_pi.integ = 0.0f; g_zsc_pi.integ = 0.0f;
        g_pi_id.integ = 0.0f; g_pi_iq.integ = 0.0f; g_pi_speed.integ = 0.0f;
        foc_startup_state = FOC_STARTUP_IDLE;
        auto_start_state = AUTO_START_IDLE;
        __set_PRIMASK(primask);
        printf("STOPPED\r\n");
    }
    else if (strcmp(cmd, "stream on") == 0)  { Stream_Enabled = 1; printf("STREAM ON\r\n");  }
    else if (strcmp(cmd, "stream off") == 0) { Stream_Enabled = 0; printf("STREAM OFF\r\n"); }
    else if (strcmp(cmd, "m0") == 0) {
        primask = __get_PRIMASK(); __disable_irq();
        oew_mode = MODE_COMP; foc_startup_state = FOC_STARTUP_IDLE; Run_Enabled = 1U;
        __set_PRIMASK(primask);
        printf("MODE COMP (Pure SPWM)\r\n");
    }
    else if (strcmp(cmd, "m1") == 0) {
        primask = __get_PRIMASK(); __disable_irq();
        oew_mode = MODE_OEW_SPLIT_SVPWM; foc_startup_state = FOC_STARTUP_IDLE; Run_Enabled = 1U;
        __set_PRIMASK(primask);
        printf("MODE OEW SPLIT SVPWM (ZSC active)\r\n");
    }
    else if (strcmp(cmd, "m2") == 0) {
        primask = __get_PRIMASK(); __disable_irq();
        oew_mode = MODE_FOC_OEW;
        foc_startup_state = FOC_STARTUP_RAMPING;
        g_pi_id.integ = 0.0f; g_pi_iq.integ = 0.0f; g_pi_speed.integ = 0.0f;
        g_foc.psi_alpha = 0.0f; g_foc.psi_beta = 0.0f; g_foc.theta = 0.0f; g_foc.omega_r = 0.0f;
        Run_Enabled = 1U;
        __set_PRIMASK(primask);
        printf("MODE FOC+OEW (Ramping...)\r\n");
    }
    else if (strcmp(cmd, "?") == 0 || strcmp(cmd, "status") == 0) {
        const char *m_str = (oew_mode == MODE_COMP) ? "COMP" :
                            (oew_mode == MODE_OEW_SPLIT_SVPWM) ? "OEW_SPLIT_SVPWM" : "FOC_OEW";
        const char *foc_str = (foc_startup_state == FOC_STARTUP_IDLE)    ? "IDLE"    :
                              (foc_startup_state == FOC_STARTUP_RAMPING) ? "RAMPING" : "RUNNING";
        printf("Mode=%s, FOC=%s, ISR_Hz=%.1f, dt=%.6f, speed=%.2f, power=%.3f, Irms=%.3f, flt=%u\r\n",
               m_str, foc_str, measured_isr_hz, control_isr_dt,
               current_speed, current_power,
               sqrtf(fmaxf(g_curr.rms2_lpf, 0.0f)), Fault_OverCurrent);
    }
    else if (strcmp(cmd, "?i") == 0) {
        float i_rms1 = sqrtf(fmaxf(g_curr.rms2_lpf, 0.0f));
        float i_rms2 = sqrtf(fmaxf(g_curr2.rms2_lpf, 0.0f));
        float rms2_avg = (g_curr.rms2_lpf + g_curr2.rms2_lpf) * 0.5f;
        float i_rms_avg = sqrtf(fmaxf(rms2_avg, 0.0f));
        float izs1 = (g_curr.ia  + g_curr.ib  + g_curr.ic)  * (1.0f / 3.0f);
        float izs2 = (g_curr2.ia + g_curr2.ib + g_curr2.ic) * (1.0f / 3.0f);
        float izs  = izs1 - izs2;
        printf("INV1 Iabc[A]: %.3f %.3f %.3f, Irms=%.3f, Izs=%.3f\r\n",
               g_curr.ia, g_curr.ib, g_curr.ic, i_rms1, izs1);
        printf("INV2 Iabc[A]: %.3f %.3f %.3f, Irms=%.3f, Izs=%.3f\r\n",
               g_curr2.ia, g_curr2.ib, g_curr2.ic, i_rms2, izs2);
        printf("AVG  Irms=%.3f, Iref=%.3f, fault=%u, cal=%u/%u\r\n",
               i_rms_avg, Target_Current_A, Fault_OverCurrent,
               g_curr.calibrated, g_curr2.calibrated);
        printf("ZSC  Izs_diff=%.4f A, PI_integ=%.4f, kp=%.4f, ki=%.4f\r\n",
               izs, g_zsc_pi.integ, g_zsc_pi.kp, g_zsc_pi.ki);
        const char *atune_state_str[] = {"IDLE", "RUNNING", "DONE", "FAILED"};
        printf("ZSC_ATUNE state=%s, half_periods=%lu, kp_result=%.4f, ki_result=%.4f\r\n",
               atune_state_str[g_zsc_atune.state],
               (unsigned long)g_zsc_atune.half_periods,
               g_zsc_atune.result_kp, g_zsc_atune.result_ki);
        printf("FOC  Id=%.3f, Iq=%.3f, Vd=%.3f, Vq=%.3f, Wr=%.2f, Theta=%.3f\r\n",
               g_foc.id, g_foc.iq, g_foc.vd, g_foc.vq,
               g_foc.omega_r / TWO_PI, g_foc.theta);
        printf("MOTOR Rs=%.3f Rr=%.3f Ls=%.5f Lr=%.5f Lm=%.5f p=%.1f\r\n",
               g_motor.Rs, g_motor.Rr, g_motor.Ls, g_motor.Lr, g_motor.Lm, g_motor.p_pairs);
    }
    else if (strcmp(cmd, "zscatune") == 0) {
        if (oew_mode != MODE_OEW_SPLIT_SVPWM && oew_mode != MODE_FOC_OEW) {
            printf("ZSC autotune: switch to m1 or m2 first\r\n");
        } else if (!g_curr.calibrated || !g_curr2.calibrated) {
            printf("ZSC autotune: ADC not calibrated yet, wait\r\n");
        } else if (g_zsc_atune.state == ZSC_ATUNE_RUNNING) {
            printf("ZSC autotune: already running (%lu half-periods)\r\n",
                   (unsigned long)g_zsc_atune.half_periods);
        } else {
            primask = __get_PRIMASK(); __disable_irq();
            g_zsc_atune = (ZscAutotune_t){ .state = ZSC_ATUNE_RUNNING, .relay_amp = ZSC_ATUNE_RELAY_AMP };
            g_zsc_pi.integ = 0.0f;
            __set_PRIMASK(primask);
            printf("ZSC autotune started. Relay amp=%.1f ticks. Min %u half-periods.\r\n",
                   ZSC_ATUNE_RELAY_AMP, ZSC_ATUNE_MIN_PERIODS);
        }
    }
    else if (cmd[0] == 's') { Target_Speed = clampf(strtof(&cmd[1], NULL), -MAX_SPEED_ELEC_HZ, MAX_SPEED_ELEC_HZ); }
    else if (cmd[0] == 'p') {
        primask = __get_PRIMASK(); __disable_irq();
        Target_Power = clampf(strtof(&cmd[1], NULL), 0.0f, MAX_POWER);
        __set_PRIMASK(primask);
    }
    else if (cmd[0] == 'i') { Target_Current_A = clampf(strtof(&cmd[1], NULL), 0.0f, I_RMS_MAX_A * 0.95f); }
    else if (cmd[0] == 'K' && cmd[1] == 'P') { g_i_pi.kp = strtof(&cmd[2], NULL); printf("KP OK\r\n"); }
    else if (cmd[0] == 'K' && cmd[1] == 'I') { g_i_pi.ki = strtof(&cmd[2], NULL); printf("KI OK\r\n"); }
    else if (cmd[0] == 'I' && cmd[1] == 'M') { I_PHASE_ABS_MAX_A = strtof(&cmd[2], NULL); printf("IMAX OK\r\n"); }
    else if (cmd[0] == 'I' && cmd[1] == 'R') { I_RMS_MAX_A = strtof(&cmd[2], NULL); printf("IRMS OK\r\n"); }
    else if (cmd[0] == 'D' && cmd[1] == 'T') { I_DIDT_MAX_PER_TICK = strtof(&cmd[2], NULL); printf("DT OK\r\n"); }
    else if (cmd[0] == 'M' && cmd[1] == 'P') {
        float rs, rr, ls, lr, lm;
        if (sscanf(&cmd[2], "%f,%f,%f,%f,%f", &rs, &rr, &ls, &lr, &lm) == 5) {
            g_motor.Rs = rs; g_motor.Rr = rr; g_motor.Ls = ls; g_motor.Lr = lr; g_motor.Lm = lm;
            printf("MOTOR PARAMS UPDATED: Rs=%.3f Rr=%.3f Ls=%.4f Lr=%.4f Lm=%.4f\r\n", rs, rr, ls, lr, lm);
        } else { printf("MP FORMAT ERROR. Use: MP<rs>,<rr>,<ls>,<lr>,<lm>\r\n"); }
    }
    else if (strcmp(cmd, "faultclr") == 0) {
        primask = __get_PRIMASK(); __disable_irq();
        Fault_OverCurrent = 0U;
        TIM1->BDTR |= TIM_BDTR_MOE; TIM8->BDTR |= TIM_BDTR_MOE;
        __set_PRIMASK(primask);
        printf("FAULT CLEARED\r\n");
    }
    else if (strcmp(cmd, "capture") == 0) {
        if (burst.busy) { printf("BUSY\r\n"); }
        else {
            primask = __get_PRIMASK(); __disable_irq();
            burst.count = 0U; burst.armed = 1U;
            burst.isr_hz = (uint32_t)measured_isr_hz;
            __set_PRIMASK(primask);
            printf("CAPTURE ARMED\r\n");
        }
    }
    else if (strcmp(cmd, "testL") == 0) {
        if (Fault_OverCurrent == 1U) { printf("Clear fault first\r\n"); }
        else if (test_l_active == 0U) {
            test_l_active = 1U; test_l_idx = 0U;
            printf("L_TEST_START\r\n");
        }
    }
    else if (strcmp(cmd, "calib") == 0) {
        primask = __get_PRIMASK(); __disable_irq();
        g_curr.calibrated = 0U; g_curr.calib_cnt = 0U;
        g_curr.off_a = 0.0f; g_curr.off_b = 0.0f; g_curr.off_c = 0.0f;
        g_curr2.calibrated = 0U; g_curr2.calib_cnt = 0U;
        g_curr2.off_a = 0.0f; g_curr2.off_b = 0.0f; g_curr2.off_c = 0.0f;
        __set_PRIMASK(primask);
        printf("CALIB RESET\r\n");
    }
    else { printf("Unknown cmd '%s'. Use: start, stop, s<val>, p<val>, i<val>, m0/m1/m2, "
                 "stream on/off, ?, ?i, zscatune, capture, testL, calib, faultclr, "
                 "KP/KI/IM/IR/DT/MP\r\n", cmd); }
}
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void uart2_tx_blocking(const uint8_t *p, uint16_t n) {
    for (uint16_t i = 0; i < n; i++) {
        uint32_t timeout = 100000;
        while (!__HAL_UART_GET_FLAG(&huart2, UART_FLAG_TXE)) {
            if (timeout-- == 0) return;
        }
        huart2.Instance->TDR = p[i];
    }
    uint32_t timeout = 5000;
    while (!__HAL_UART_GET_FLAG(&huart2, UART_FLAG_TC)) {
        if (timeout-- == 0) return;
    }
}

void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance != USART2) { return; }
    char c = (char)uart_rx_byte;
    if (!uart_rx_ready) {
        if ((c == '\r') || (c == '\n')) {
            if (uart_rx_len > 0U) {
                uart_rx_buf[uart_rx_len] = '\0';
                uart_rx_ready = 1U;
            }
        } else if (uart_rx_len < (UART_RX_BUF_SIZE - 1U)) {
            uart_rx_buf[uart_rx_len] = c;
            uart_rx_len++;
        }
    }
    HAL_UART_Receive_IT(&huart2, &uart_rx_byte, 1U);
}

void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance == USART2) {
        __HAL_UART_CLEAR_OREFLAG(huart);
        HAL_UART_Receive_IT(&huart2, &uart_rx_byte, 1U);
    }
}
/* ========== ISR: ADC Injected Conv Complete ==========
 * Сердце FOC. Вызывается с частотой PWM / (2 * RepetitionCounter).
 * При RepetitionCounter=1 и PWM=5 кГц -> ISR=5 кГц.
 */
void HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef* hadc) {
    if (hadc->Instance != ADC1) return;
    isr_tick_count++;

    /* --- testL: измерение индуктивности (импульсный тест) --- */
    if (test_l_active) {
        if (test_l_idx < TEST_L_SAMPLES) {
            uint32_t test_duty = HALF_PERIOD + 1700;
            TIM1->CCR1 = test_duty; TIM1->CCR2 = PWM_PERIOD - test_duty; TIM1->CCR3 = HALF_PERIOD;
            TIM8->CCR1 = PWM_PERIOD - test_duty; TIM8->CCR2 = test_duty; TIM8->CCR3 = PWM_PERIOD - HALF_PERIOD;
            test_l_ia[test_l_idx] = (int16_t)(g_curr.ia * 1000.0f);
            test_l_idx++;
        } else {
            TIM1->CCR1 = HALF_PERIOD; TIM1->CCR2 = HALF_PERIOD; TIM1->CCR3 = HALF_PERIOD;
            TIM8->CCR1 = HALF_PERIOD; TIM8->CCR2 = HALF_PERIOD; TIM8->CCR3 = HALF_PERIOD;
            test_l_active = 0U;
            print_testl = 1U;
        }
        return;
    }

    /* --- Чтение инжектированных каналов ADC1+ADC2 (синхронно) --- */
    float a1 = (float)hadc1.Instance->JDR1;
    float b1 = (float)hadc1.Instance->JDR2;
    float c1_raw = (float)hadc1.Instance->JDR3;   /* ADC1 IN6 (PC0) - для отладки */

    float a2 = (float)hadc2.Instance->JDR1;
    float b2 = (float)hadc2.Instance->JDR2;
    float c2_raw = (float)hadc2.Instance->JDR3;   /* ADC2 IN4 - для отладки */

    /* Третья фаза вычисляется по Кирхгофу (проверенный метод) */
    float zero_sum1 = g_curr.calibrated
        ? (g_curr.off_a + g_curr.off_b + g_curr.off_c)
        : 4096.0f;
    float c1 = zero_sum1 - a1 - b1;

    float zero_sum2 = g_curr2.calibrated
        ? (g_curr2.off_a + g_curr2.off_b + g_curr2.off_c)
        : 4096.0f;
    float c2 = zero_sum2 - a2 - b2;

    last_raw_a1 = (uint16_t)a1; last_raw_b1 = (uint16_t)b1; last_raw_c1 = (uint16_t)c1_raw;
    last_raw_a2 = (uint16_t)a2; last_raw_b2 = (uint16_t)b2; last_raw_c2 = (uint16_t)c2_raw;

    current_update(&g_curr,  a1, b1, c1);
    current_update(&g_curr2, a2, b2, c2);

    /* --- Защитные проверки (только после калибровки) --- */
    float target_speed = isfinite(Target_Speed) ? clampf(Target_Speed, -MAX_SPEED_ELEC_HZ, MAX_SPEED_ELEC_HZ) : 0.0f;
    float target_power = isfinite(Target_Power) ? clampf(Target_Power, 0.0f, MAX_POWER) : 0.0f;
    float dt = control_isr_dt;

    if (g_curr.calibrated) {
        float di_a = fabsf(g_curr.ia - i_prev_a);
        float di_b = fabsf(g_curr.ib - i_prev_b);
        float di_c = fabsf(g_curr.ic - i_prev_c);
        i_prev_a = g_curr.ia; i_prev_b = g_curr.ib; i_prev_c = g_curr.ic;
        if ((di_a > I_DIDT_MAX_PER_TICK) || (di_b > I_DIDT_MAX_PER_TICK) || (di_c > I_DIDT_MAX_PER_TICK)) {
            pwm_shutdown_fault(); return;
        }
        float i_abs = fmaxf(fmaxf(fabsf(g_curr.ia), fabsf(g_curr.ib)), fabsf(g_curr.ic));
        float i_rms = sqrtf(fmaxf(g_curr.rms2_lpf, 0.0f));
        if ((i_abs > I_PHASE_ABS_MAX_A) || (i_rms > I_RMS_MAX_A)) { pwm_shutdown_fault(); return; }

        /* Внешний контур: ток -> мощность */
        if (Fault_OverCurrent == 0U && Run_Enabled) {
            float err = clampf(Target_Current_A, 0.0f, I_RMS_MAX_A * 0.95f) - i_rms;
            target_power = pi_step(&g_i_pi, err, dt);
            Target_Power = clampf(target_power, 0.0f, MAX_POWER);
        } else {
            target_power = 0.0f;
            Target_Power = 0.0f;
            g_i_pi.integ = 0.0f;   /* anti-windup на останове */
        }
    }

    /* --- Инициализация duties нейтральным вектором (50%) --- */
    uint32_t d1a = HALF_PERIOD, d1b = HALF_PERIOD, d1c = HALF_PERIOD;
    uint32_t d2a = HALF_PERIOD, d2b = HALF_PERIOD, d2c = HALF_PERIOD;

    if (oew_mode == MODE_FOC_OEW) {
        /* ============ FOC режим ============ */
        if (foc_startup_state == FOC_STARTUP_RAMPING) {
            /* Разгон в V/f до частоты входа в FOC, затем переход в RUNNING */
            float ramp_target_speed = fmaxf(fabsf(target_speed), FOC_MIN_STARTUP_HZ);
            if (target_speed < 0.0f) ramp_target_speed = -ramp_target_speed;

            if (current_speed < ramp_target_speed) {
                current_speed += SPEED_RATE * dt;
                if (current_speed > ramp_target_speed) current_speed = ramp_target_speed;
            } else if (current_speed > ramp_target_speed) {
                current_speed -= SPEED_RATE * dt;
                if (current_speed < ramp_target_speed) current_speed = ramp_target_speed;
            }

            current_power = Target_Power;
            ugol1 += TWO_PI * current_speed * dt;
            ugol1 = fmodf(ugol1, TWO_PI); if (ugol1 < 0.0f) ugol1 += TWO_PI;

            float amp = current_power * (float)PWM_PERIOD * 0.5f;
            float va = amp * CORDIC_Cos(ugol1);
            float vb = amp * CORDIC_Cos(ugol1 - 2.0943951f);
            float vc = amp * CORDIC_Cos(ugol1 + 2.0943951f);

            float v1a = 0.5f * va, v1b = 0.5f * vb, v1c = 0.5f * vc;
            float v2a = -0.5f * va, v2b = -0.5f * vb, v2c = -0.5f * vc;

            float izs = 0.0f;
            if (g_curr.calibrated && g_curr2.calibrated) {
                izs = ((g_curr.ia + g_curr.ib + g_curr.ic) * 0.333333f)
                    - ((g_curr2.ia + g_curr2.ib + g_curr2.ic) * 0.333333f);
            }
            izs_val = izs;
            float vzs_corr = (g_zsc_atune.state == ZSC_ATUNE_RUNNING)
                ? zsc_autotune_step(&g_zsc_atune, izs, dt)
                : pi_step(&g_zsc_pi, -izs, dt);

            d1a = clamp_u32_from_float((float)HALF_PERIOD + v1a - vzs_corr, 0U, PWM_PERIOD);
            d1b = clamp_u32_from_float((float)HALF_PERIOD + v1b - vzs_corr, 0U, PWM_PERIOD);
            d1c = clamp_u32_from_float((float)HALF_PERIOD + v1c - vzs_corr, 0U, PWM_PERIOD);
            d2a = clamp_u32_from_float((float)HALF_PERIOD + v2a + vzs_corr, 0U, PWM_PERIOD);
            d2b = clamp_u32_from_float((float)HALF_PERIOD + v2b + vzs_corr, 0U, PWM_PERIOD);
            d2c = clamp_u32_from_float((float)HALF_PERIOD + v2c + vzs_corr, 0U, PWM_PERIOD);

            /* При достижении пороговой скорости - переход в полноценный FOC */
            if (fabsf(current_speed) >= FOC_MIN_STARTUP_HZ * 0.95f) {
                float ialpha0, ibeta0;
                Clarke_Transform(g_curr.ia, g_curr.ib, &ialpha0, &ibeta0);
                g_foc.theta    = ugol1;
                g_foc.omega_r  = TWO_PI * current_speed;
                g_foc.psi_alpha = g_motor.Lm * ialpha0;
                g_foc.psi_beta  = g_motor.Lm * ibeta0;
                foc_startup_state = FOC_STARTUP_RUNNING;
                print_foc_engaged = 1U;
            }
        }
        else if (foc_startup_state == FOC_STARTUP_RUNNING) {
            /* --- Полный FOC: Clarke -> Park -> PI(Id,Iq) -> Inv-Park -> Inv-Clarke --- */
            float Id_target = Target_Power * 2.0f;

#if FIELD_WEAKENING_ENABLE
            float omega_r_hz = g_foc.omega_r / TWO_PI;
            Id_target = field_weakening_id_target(omega_r_hz, Id_target);
#endif
            float Iq_target = pi_step(&g_pi_speed, target_speed - (g_foc.omega_r / TWO_PI), dt);

            float ialpha, ibeta;
            Clarke_Transform(g_curr.ia, g_curr.ib, &ialpha, &ibeta);
            RotorFluxObserver(&g_foc, &g_motor, ialpha, ibeta, dt);

            float sin_t = CORDIC_Sin(g_foc.theta);
            float cos_t = CORDIC_Cos(g_foc.theta);
            Park_Transform(ialpha, ibeta, sin_t, cos_t, &g_foc.id, &g_foc.iq);

            g_foc.vd = pi_step(&g_pi_id, Id_target - g_foc.id, dt);
            g_foc.vq = pi_step(&g_pi_iq, Iq_target - g_foc.iq, dt);

#if RS_DROP_COMP_ENABLE
            /* Низкоскоростная компенсация падения на stator resistance: V_rs = Rs * Id */
            float v_rs = g_motor.Rs * g_foc.id;
            g_foc.vd += v_rs;
#endif

            float valpha, vbeta;
            Inv_Park_Transform(g_foc.vd, g_foc.vq, sin_t, cos_t, &valpha, &vbeta);

            /* ИСПРАВЛЕНО: вызываем корректный Inv-Clarke (раньше был copy-paste из прямого) */
            float va_norm, vb_norm, vc_norm;
            Inv_Clarke_Transform(valpha, vbeta, &va_norm, &vb_norm, &vc_norm);

            /* ZSC PI: компенсация тока нулевой последовательности между инверторами */
            float izs = ((g_curr.ia + g_curr.ib + g_curr.ic) * 0.333333f)
                      - ((g_curr2.ia + g_curr2.ib + g_curr2.ic) * 0.333333f);
            izs_val = izs;
            float vzs_corr = (g_zsc_atune.state == ZSC_ATUNE_RUNNING)
                ? zsc_autotune_step(&g_zsc_atune, izs, dt)
                : pi_step(&g_zsc_pi, -izs, dt);

            /* OEW: половина напряжения на каждый инвертор, ZSC-поправка в противофазе */
            float va_duty = va_norm * (float)HALF_PERIOD;
            float vb_duty = vb_norm * (float)HALF_PERIOD;
            float vc_duty = vc_norm * (float)HALF_PERIOD;

            d1a = clamp_u32_from_float((float)HALF_PERIOD + (0.5f * va_duty) - vzs_corr, 0U, PWM_PERIOD);
            d1b = clamp_u32_from_float((float)HALF_PERIOD + (0.5f * vb_duty) - vzs_corr, 0U, PWM_PERIOD);
            d1c = clamp_u32_from_float((float)HALF_PERIOD + (0.5f * vc_duty) - vzs_corr, 0U, PWM_PERIOD);
            d2a = clamp_u32_from_float((float)HALF_PERIOD - (0.5f * va_duty) + vzs_corr, 0U, PWM_PERIOD);
            d2b = clamp_u32_from_float((float)HALF_PERIOD - (0.5f * vb_duty) + vzs_corr, 0U, PWM_PERIOD);
            d2c = clamp_u32_from_float((float)HALF_PERIOD - (0.5f * vc_duty) + vzs_corr, 0U, PWM_PERIOD);

            ugol1 = g_foc.theta;

            /* При потере скорости - откат к V/f ramping */
            if (fabsf(g_foc.omega_r / TWO_PI) < FOC_LOST_HZ) {
                foc_startup_state = FOC_STARTUP_RAMPING;
                print_foc_lost = 1U;
            }
        }
        else {
            /* FOC_STARTUP_IDLE: нейтральный вектор 50% (d1a-d2c уже инициализированы) */
        }
    }
    else {
        /* ============ V/f режим (COMP или OEW_SPLIT_SVPWM) ============ */
        if (current_speed < target_speed) {
            current_speed += SPEED_RATE * dt;
            if (current_speed > target_speed) current_speed = target_speed;
        } else if (current_speed > target_speed) {
            current_speed -= SPEED_RATE * dt;
            if (current_speed < target_speed) current_speed = target_speed;
        }
        if (current_power < target_power) {
            current_power += POWER_RATE * dt;
            if (current_power > target_power) current_power = target_power;
        } else if (current_power > target_power) {
            current_power -= POWER_RATE * dt;
            if (current_power < target_power) current_power = target_power;
        }

        ugol1 += TWO_PI * current_speed * dt;
        ugol1 = fmodf(ugol1, TWO_PI); if (ugol1 < 0.0f) ugol1 += TWO_PI;

        float amp = current_power * (float)PWM_PERIOD * 0.5f;
        float va = amp * CORDIC_Cos(ugol1);
        float vb = amp * CORDIC_Cos(ugol1 - 2.0943951f);
        float vc = amp * CORDIC_Cos(ugol1 + 2.0943951f);

        if (oew_mode == MODE_COMP) {
            /* COMP: чистая синусоида, TIM8 = инверсия TIM1 */
            d1a = clamp_u32_from_float((float)HALF_PERIOD + va, 0U, PWM_PERIOD);
            d1b = clamp_u32_from_float((float)HALF_PERIOD + vb, 0U, PWM_PERIOD);
            d1c = clamp_u32_from_float((float)HALF_PERIOD + vc, 0U, PWM_PERIOD);
            d2a = PWM_PERIOD - d1a; d2b = PWM_PERIOD - d1b; d2c = PWM_PERIOD - d1c;
        } else {
            /* OEW_SPLIT_SVPWM: разделённое напряжение + ZSC PI */
            float v1a = 0.5f * va, v1b = 0.5f * vb, v1c = 0.5f * vc;
            float v2a = -0.5f * va, v2b = -0.5f * vb, v2c = -0.5f * vc;

            float off1 = 0.0f, off2 = 0.0f;
            float izs = 0.0f;
            if (g_curr.calibrated && g_curr2.calibrated) {
                float izs1 = (g_curr.ia + g_curr.ib + g_curr.ic) * 0.333333f;
                float izs2 = (g_curr2.ia + g_curr2.ib + g_curr2.ic) * 0.333333f;
                izs = izs1 - izs2;
            }
            izs_val = izs;

            float vzs_corr = (g_zsc_atune.state == ZSC_ATUNE_RUNNING)
                ? zsc_autotune_step(&g_zsc_atune, izs, dt)
                : pi_step(&g_zsc_pi, -izs, dt);
            off1 += vzs_corr; off2 -= vzs_corr;

            d1a = clamp_u32_from_float((float)HALF_PERIOD + (v1a - off1), 0U, PWM_PERIOD);
            d1b = clamp_u32_from_float((float)HALF_PERIOD + (v1b - off1), 0U, PWM_PERIOD);
            d1c = clamp_u32_from_float((float)HALF_PERIOD + (v1c - off1), 0U, PWM_PERIOD);
            d2a = clamp_u32_from_float((float)HALF_PERIOD + (v2a - off2), 0U, PWM_PERIOD);
            d2b = clamp_u32_from_float((float)HALF_PERIOD + (v2b - off2), 0U, PWM_PERIOD);
            d2c = clamp_u32_from_float((float)HALF_PERIOD + (v2c - off2), 0U, PWM_PERIOD);
        }
    }

    /* --- Применение duties к таймерам --- */
    TIM1->CCR1 = d1a; TIM1->CCR2 = d1b; TIM1->CCR3 = d1c;
    TIM8->CCR1 = d2a; TIM8->CCR2 = d2b; TIM8->CCR3 = d2c;

    /* --- Burst capture (если вооружён) --- */
    if (burst.armed) {
        uint16_t i = burst.count;
        if (i < BURST_CAP_SIZE) {
            BurstSample_t *s = &burst_buf[i];
            s->ia1 = (int16_t)(g_curr.ia  * 1000.0f);
            s->ib1 = (int16_t)(g_curr.ib  * 1000.0f);
            s->ic1 = (int16_t)(g_curr.ic  * 1000.0f);
            s->ia2 = (int16_t)(g_curr2.ia * 1000.0f);
            s->ib2 = (int16_t)(g_curr2.ib * 1000.0f);
            s->ic2 = (int16_t)(g_curr2.ic * 1000.0f);
            s->izs = (int16_t)(izs_val     * 1000.0f);
            s->ang = (uint16_t)(ugol1 * (65536.0f / TWO_PI));
            s->d1a = (uint16_t)d1a; s->d1b = (uint16_t)d1b; s->d1c = (uint16_t)d1c;
            s->d2a = (uint16_t)d2a; s->d2b = (uint16_t)d2b; s->d2c = (uint16_t)d2c;
            burst.count = i + 1U;
            if ((i + 1U) >= BURST_CAP_SIZE) { burst.armed = 0U; burst.busy = 1U; }
        }
    }
}

int __io_putchar(int ch) {
    HAL_UART_Transmit(&huart2, (uint8_t*)&ch, 1, HAL_MAX_DELAY);
    return ch;
}

/* __io_getchar намеренно отключён: syscalls.c::_read дёргает его через stdin-буфер Newlib,
   а HAL_UART_Receive здесь блокирующий — он съедает байты, предназначенные для IT-приёма команд. */
int __io_getchar(void) {
    return (int)0;
}

/**
  * @brief  The application entry point.
  * @retval int
  */
int main(void)
{
  /* MCU Configuration--------------------------------------------------------*/
  HAL_Init();

  /* Configure the system clock */
  SystemClock_Config();

  /* Initialize all configured peripherals */
  MX_GPIO_Init();
  MX_ADC1_Init();
  MX_ADC2_Init();
  MX_TIM1_Init();
  MX_TIM8_Init();
  MX_CORDIC_Init();
  MX_USART2_UART_Init();

  /* USER CODE BEGIN 2 */

#if IWDG_WATCHDOG_ENABLE
  /* IWDG: independent watchdog. ~512 мс таймаут. Если main зависнет - сброс. */
  IWDG_HandleTypeDef hiwdg;
  hiwdg.Instance = IWDG;
  hiwdg.Init.Prescaler = IWDG_PRESCALER_64;
  hiwdg.Init.Reload = 4095;
  hiwdg.Init.Window = 4095;
  if (HAL_IWDG_Init(&hiwdg) != HAL_OK) { Error_Handler(); }
#endif

  /* Из Проекта 2: аппаратная калибровка ADC при старте (в дополнение к рантайм-offset) */
  (void)HAL_ADCEx_Calibration_Start(&hadc1, ADC_SINGLE_ENDED);
  (void)HAL_ADCEx_Calibration_Start(&hadc2, ADC_SINGLE_ENDED);

  /* 1. Запускаем ШИМ на обоих таймерах (CHx + CHxN) */
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1);
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_2);
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_3);
  HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_1);
  HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_2);
  HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_3);

  HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_1);
  HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_2);
  HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_3);
  HAL_TIMEx_PWMN_Start(&htim8, TIM_CHANNEL_1);
  HAL_TIMEx_PWMN_Start(&htim8, TIM_CHANNEL_2);
  HAL_TIMEx_PWMN_Start(&htim8, TIM_CHANNEL_3);

  /* 2. Включаем главный выход (Main Output Enable) */
  __HAL_TIM_MOE_ENABLE(&htim1);
  __HAL_TIM_MOE_ENABLE(&htim8);

  /* 3. Запускаем инжектированные измерения АЦП с прерываниями */
  HAL_ADCEx_InjectedStart_IT(&hadc1);
  HAL_ADCEx_InjectedStart(&hadc2);

  /* 4. Запускаем прием данных по UART */
  HAL_UART_Receive_IT(&huart2, &uart_rx_byte, 1U);
  SET_BIT(USART2->CR1, USART_CR1_RXNEIE);  /* страховка: явно включаем RXNE IT */

  printf("System Started. PWM & ADC Enabled. Combined FOC+OEW build.\r\n");
  /* USER CODE END 2 */

  /* Initialize led */
  BSP_LED_Init(LED_GREEN);

  /* Initialize USER push-button */
  BSP_PB_Init(BUTTON_USER, BUTTON_MODE_EXTI);

  /* -- Sample board code to switch on led ---- */
  BSP_LED_On(LED_GREEN);

  /* USER CODE BEGIN WHILE */
  static uint32_t last_tick = 0;
  static uint32_t diag_last_isr_count = 0U;
  static ZscAtuneState_t atune_state_prev = ZSC_ATUNE_IDLE;

  while (1)
  {
    /* -- User push-button -- */
    if (BspButtonState == BUTTON_PRESSED) {
        BspButtonState = BUTTON_RELEASED;
        BSP_LED_Toggle(LED_GREEN);
    }

    /* Обработка входящих команд с терминала */
    if (uart_rx_ready) {
        char cmd_copy[UART_RX_BUF_SIZE];
        uint32_t primask = __get_PRIMASK(); __disable_irq();
        memcpy(cmd_copy, (const void*)uart_rx_buf, uart_rx_len + 1U);
        uart_rx_len = 0U;
        uart_rx_ready = 0U;
        __set_PRIMASK(primask);
        process_uart_command(cmd_copy);
    }

    /* Обработка автозапуска: calib -> spin -> ZSC autotune -> restore targets */
    switch (auto_start_state) {
        case AUTO_START_IDLE:
            break;

        case AUTO_START_CALIB:
            if (g_curr.calibrated && g_curr2.calibrated) {
                auto_start_state = AUTO_START_SPIN;
                auto_start_t0 = HAL_GetTick();
                Run_Enabled = 1U;
                printf("AUTO START: Spinning up...\r\n");
            } else if (HAL_GetTick() - auto_start_t0 > AUTO_CALIB_TIMEOUT_MS) {
                printf("AUTO START: calib timeout\r\n");
                auto_start_state = AUTO_START_IDLE;
            }
            break;

        case AUTO_START_SPIN:
            if (HAL_GetTick() - auto_start_t0 > AUTO_SPIN_TIME_MS) {
                uint32_t primask = __get_PRIMASK(); __disable_irq();
                g_zsc_atune = (ZscAutotune_t){ .state = ZSC_ATUNE_RUNNING, .relay_amp = ZSC_ATUNE_RELAY_AMP };
                g_zsc_pi.integ = 0.0f;
                __set_PRIMASK(primask);
                auto_start_state = AUTO_START_ZSC;
                auto_start_t0 = HAL_GetTick();
                printf("AUTO START: ZSC autotune...\r\n");
            }
            break;

        case AUTO_START_ZSC:
            if (g_zsc_atune.state == ZSC_ATUNE_DONE) {
                auto_start_state = AUTO_START_DONE;
                printf("AUTO START: ZSC tuned (kp=%.4f, ki=%.4f), restoring targets\r\n",
                       g_zsc_atune.result_kp, g_zsc_atune.result_ki);
            } else if (g_zsc_atune.state == ZSC_ATUNE_FAILED ||
                       (HAL_GetTick() - auto_start_t0 > AUTO_ZSC_TIMEOUT_MS)) {
                printf("AUTO START: ZSC autotune failed/timeout\r\n");
                auto_start_state = AUTO_START_IDLE;
                Run_Enabled = 0U;
            }
            break;

        case AUTO_START_DONE: {
            uint32_t primask = __get_PRIMASK(); __disable_irq();
            Target_Speed = autosave_speed;
            Target_Power = autosave_power;
            Target_Current_A = autosave_current;
            oew_mode = autosave_mode;
            if (autosave_mode == MODE_FOC_OEW) {
                foc_startup_state = FOC_STARTUP_RAMPING;
                g_pi_id.integ = 0.0f; g_pi_iq.integ = 0.0f; g_pi_speed.integ = 0.0f;
                g_foc.psi_alpha = 0.0f; g_foc.psi_beta = 0.0f; g_foc.theta = 0.0f; g_foc.omega_r = 0.0f;
            }
            __set_PRIMASK(primask);
            auto_start_state = AUTO_START_IDLE;
            printf("AUTO START: DONE\r\n");
            break;
        }
    }

    /* Безопасный вывод сообщений из ISR (перенесён из ISR в main loop) */
    if (print_testl) {
        print_testl = 0U;
        printf("$TESTL_BEGIN\r\n");
        for (int i = 0; i < TEST_L_SAMPLES; i++) { printf("%d,%d\r\n", i, test_l_ia[i]); }
        printf("$TESTL_END\r\n");
    }
    if (print_foc_engaged) { print_foc_engaged = 0U; printf("FOC ENGAGED\r\n"); }
    if (print_foc_lost)    { print_foc_lost    = 0U; printf("FOC LOST (Low speed) -> Ramping\r\n"); }

    /* Отправка потока данных (JSON), если включён стрим */
    if (Stream_Enabled) {
        UART_Send_Stream();
        HAL_Delay(20);
    }

    /* Выгрузка захваченных данных (Capture) */
    if (burst.busy) { burst_dump_step(); }

    /* Из Проекта 2: измерение ISR_Hz каждую секунду + обновление control_isr_dt.
     * Также - LED-индикатор: горит при ISR_Hz в допуске, мигает при отклонении. */
    uint32_t current_tick = HAL_GetTick();
    if (current_tick - last_tick >= 1000) {
        uint32_t primask = __get_PRIMASK(); __disable_irq();
        uint32_t ticks = isr_tick_count;
        isr_tick_count = 0U;
        __set_PRIMASK(primask);

        uint32_t delta_count = ticks;
        uint32_t elapsed_ms = current_tick - last_tick;
        float hz = ((float)delta_count * 1000.0f) / (float)elapsed_ms;
        if ((hz > 100.0f) && isfinite(hz)) {
            measured_isr_hz = hz;
            float new_dt = 1.0f / hz;
            __disable_irq();
            control_isr_dt = new_dt;
            __enable_irq();
        }

        float low  = EXPECTED_ISR_HZ * (1.0f - ISR_HZ_TOLERANCE);
        float high = EXPECTED_ISR_HZ * (1.0f + ISR_HZ_TOLERANCE);
        if ((measured_isr_hz >= low) && (measured_isr_hz <= high)) {
            BSP_LED_On(LED_GREEN);
        } else {
            BSP_LED_Toggle(LED_GREEN);
        }

        /* Контроль изменения состояния ZSC autotune (для оповещения пользователя) */
        ZscAtuneState_t atune_state_now = g_zsc_atune.state;
        if (atune_state_now != atune_state_prev) {
            if (atune_state_now == ZSC_ATUNE_DONE) {
                printf(">>> ZSC autotune DONE: kp=%.4f, ki=%.4f\r\n", g_zsc_atune.result_kp, g_zsc_atune.result_ki);
            } else if (atune_state_now == ZSC_ATUNE_FAILED) {
                printf(">>> ZSC autotune FAILED: timeout. Check motor running and mode.\r\n");
            }
            atune_state_prev = atune_state_now;
        }

        last_tick = current_tick;
        diag_last_isr_count = ticks;
    }

#if IWDG_WATCHDOG_ENABLE
    HAL_IWDG_Refresh(&hiwdg);
#endif

    /* USER CODE END WHILE */
  }
  /* USER CODE END 3 */
}
/* ========== SystemClock_Config ==========
 * HSE=24 MHz, PLLM=6, PLLN=85, PLLR=2 -> f_VCO=340 MHz, f_SYS=170 MHz.
 * Корректно для STM32G4 (VCO в [128, 536] МГц, SYS <= 170 МГц).
 */
void SystemClock_Config(void)
{
  RCC_OscInitTypeDef RCC_OscInitStruct = {0};
  RCC_ClkInitTypeDef RCC_ClkInitStruct = {0};

  HAL_PWREx_ControlVoltageScaling(PWR_REGULATOR_VOLTAGE_SCALE1_BOOST);

  RCC_OscInitStruct.OscillatorType = RCC_OSCILLATORTYPE_HSE;
  RCC_OscInitStruct.HSEState = RCC_HSE_ON;
  RCC_OscInitStruct.PLL.PLLState = RCC_PLL_ON;
  RCC_OscInitStruct.PLL.PLLSource = RCC_PLLSOURCE_HSE;
  RCC_OscInitStruct.PLL.PLLM = RCC_PLLM_DIV6;
  RCC_OscInitStruct.PLL.PLLN = 85;
  RCC_OscInitStruct.PLL.PLLP = RCC_PLLP_DIV2;
  RCC_OscInitStruct.PLL.PLLQ = RCC_PLLQ_DIV2;
  RCC_OscInitStruct.PLL.PLLR = RCC_PLLR_DIV2;
  if (HAL_RCC_OscConfig(&RCC_OscInitStruct) != HAL_OK) { Error_Handler(); }

  RCC_ClkInitStruct.ClockType = RCC_CLOCKTYPE_HCLK|RCC_CLOCKTYPE_SYSCLK
                              |RCC_CLOCKTYPE_PCLK1|RCC_CLOCKTYPE_PCLK2;
  RCC_ClkInitStruct.SYSCLKSource = RCC_SYSCLKSOURCE_PLLCLK;
  RCC_ClkInitStruct.AHBCLKDivider = RCC_SYSCLK_DIV1;
  RCC_ClkInitStruct.APB1CLKDivider = RCC_HCLK_DIV1;
  RCC_ClkInitStruct.APB2CLKDivider = RCC_HCLK_DIV1;

  if (HAL_RCC_ClockConfig(&RCC_ClkInitStruct, FLASH_LATENCY_4) != HAL_OK) { Error_Handler(); }
}

/* ========== MX_ADC1_Init ==========
 * ADC1: 3 инжектированных канала (CH1, CH5, CH6) + dual-mode INJECSIMULT с ADC2.
 * Триггер - TIM1 TRGO (UPDATE event), что даёт синхронное измерение в центре PWM.
 */
static void MX_ADC1_Init(void)
{
  ADC_MultiModeTypeDef multimode = {0};
  ADC_InjectionConfTypeDef sConfigInjected = {0};

  hadc1.Instance = ADC1;
  hadc1.Init.ClockPrescaler = ADC_CLOCK_SYNC_PCLK_DIV4;
  hadc1.Init.Resolution = ADC_RESOLUTION_12B;
  hadc1.Init.DataAlign = ADC_DATAALIGN_RIGHT;
  hadc1.Init.GainCompensation = 0;
  hadc1.Init.ScanConvMode = ADC_SCAN_ENABLE;
  hadc1.Init.EOCSelection = ADC_EOC_SINGLE_CONV;
  hadc1.Init.LowPowerAutoWait = DISABLE;
  hadc1.Init.ContinuousConvMode = DISABLE;
  hadc1.Init.NbrOfConversion = 1;
  hadc1.Init.DiscontinuousConvMode = DISABLE;
  hadc1.Init.DMAContinuousRequests = DISABLE;
  hadc1.Init.Overrun = ADC_OVR_DATA_PRESERVED;
  hadc1.Init.OversamplingMode = DISABLE;
  if (HAL_ADC_Init(&hadc1) != HAL_OK) { Error_Handler(); }

  multimode.Mode = ADC_DUALMODE_INJECSIMULT;
  multimode.DMAAccessMode = ADC_DMAACCESSMODE_DISABLED;
  multimode.TwoSamplingDelay = ADC_TWOSAMPLINGDELAY_1CYCLE;
  if (HAL_ADCEx_MultiModeConfigChannel(&hadc1, &multimode) != HAL_OK) { Error_Handler(); }

  sConfigInjected.InjectedChannel = ADC_CHANNEL_1;
  sConfigInjected.InjectedRank = ADC_INJECTED_RANK_1;
  sConfigInjected.InjectedSamplingTime = ADC_SAMPLETIME_12CYCLES_5;
  sConfigInjected.InjectedSingleDiff = ADC_SINGLE_ENDED;
  sConfigInjected.InjectedOffsetNumber = ADC_OFFSET_NONE;
  sConfigInjected.InjectedOffset = 0;
  sConfigInjected.InjectedNbrOfConversion = 3;
  sConfigInjected.InjectedDiscontinuousConvMode = DISABLE;
  sConfigInjected.AutoInjectedConv = DISABLE;
  sConfigInjected.QueueInjectedContext = DISABLE;
  sConfigInjected.ExternalTrigInjecConv = ADC_EXTERNALTRIGINJEC_T1_TRGO;
  sConfigInjected.ExternalTrigInjecConvEdge = ADC_EXTERNALTRIGINJECCONV_EDGE_RISING;
  sConfigInjected.InjecOversamplingMode = DISABLE;
  if (HAL_ADCEx_InjectedConfigChannel(&hadc1, &sConfigInjected) != HAL_OK) { Error_Handler(); }

  sConfigInjected.InjectedChannel = ADC_CHANNEL_5;
  sConfigInjected.InjectedRank = ADC_INJECTED_RANK_2;
  if (HAL_ADCEx_InjectedConfigChannel(&hadc1, &sConfigInjected) != HAL_OK) { Error_Handler(); }

  sConfigInjected.InjectedChannel = ADC_CHANNEL_6;
  sConfigInjected.InjectedRank = ADC_INJECTED_RANK_3;
  if (HAL_ADCEx_InjectedConfigChannel(&hadc1, &sConfigInjected) != HAL_OK) { Error_Handler(); }
}

/* ========== MX_ADC2_Init ==========
 * ADC2: 3 инжектированных канала (CH2, CH3, CH4) - ведомый в dual-mode.
 * + 1 regular channel (CH2) для возможного использования regular-trigger.
 */
static void MX_ADC2_Init(void)
{
  ADC_InjectionConfTypeDef sConfigInjected = {0};
  ADC_ChannelConfTypeDef sConfig = {0};

  hadc2.Instance = ADC2;
  hadc2.Init.ClockPrescaler = ADC_CLOCK_SYNC_PCLK_DIV4;
  hadc2.Init.Resolution = ADC_RESOLUTION_12B;
  hadc2.Init.DataAlign = ADC_DATAALIGN_RIGHT;
  hadc2.Init.GainCompensation = 0;
  hadc2.Init.ScanConvMode = ADC_SCAN_ENABLE;
  hadc2.Init.EOCSelection = ADC_EOC_SINGLE_CONV;
  hadc2.Init.LowPowerAutoWait = DISABLE;
  hadc2.Init.ContinuousConvMode = DISABLE;
  hadc2.Init.NbrOfConversion = 1;
  hadc2.Init.DiscontinuousConvMode = DISABLE;
  hadc2.Init.DMAContinuousRequests = DISABLE;
  hadc2.Init.Overrun = ADC_OVR_DATA_PRESERVED;
  hadc2.Init.OversamplingMode = DISABLE;
  if (HAL_ADC_Init(&hadc2) != HAL_OK) { Error_Handler(); }

  sConfigInjected.InjectedChannel = ADC_CHANNEL_2;
  sConfigInjected.InjectedRank = ADC_INJECTED_RANK_1;
  sConfigInjected.InjectedSamplingTime = ADC_SAMPLETIME_12CYCLES_5;
  sConfigInjected.InjectedSingleDiff = ADC_SINGLE_ENDED;
  sConfigInjected.InjectedOffsetNumber = ADC_OFFSET_NONE;
  sConfigInjected.InjectedOffset = 0;
  sConfigInjected.InjectedNbrOfConversion = 3;
  sConfigInjected.InjectedDiscontinuousConvMode = DISABLE;
  sConfigInjected.AutoInjectedConv = DISABLE;
  sConfigInjected.QueueInjectedContext = DISABLE;
  sConfigInjected.InjecOversamplingMode = DISABLE;
  if (HAL_ADCEx_InjectedConfigChannel(&hadc2, &sConfigInjected) != HAL_OK) { Error_Handler(); }

  sConfigInjected.InjectedChannel = ADC_CHANNEL_3;
  sConfigInjected.InjectedRank = ADC_INJECTED_RANK_2;
  if (HAL_ADCEx_InjectedConfigChannel(&hadc2, &sConfigInjected) != HAL_OK) { Error_Handler(); }

  sConfigInjected.InjectedChannel = ADC_CHANNEL_4;
  sConfigInjected.InjectedRank = ADC_INJECTED_RANK_3;
  if (HAL_ADCEx_InjectedConfigChannel(&hadc2, &sConfigInjected) != HAL_OK) { Error_Handler(); }

  sConfig.Channel = ADC_CHANNEL_2;
  sConfig.Rank = ADC_REGULAR_RANK_1;
  sConfig.SamplingTime = ADC_SAMPLETIME_12CYCLES_5;
  sConfig.SingleDiff = ADC_SINGLE_ENDED;
  sConfig.OffsetNumber = ADC_OFFSET_NONE;
  sConfig.Offset = 0;
  if (HAL_ADC_ConfigChannel(&hadc2, &sConfig) != HAL_OK) { Error_Handler(); }
}

/* ========== MX_CORDIC_Init ==========
 * CORDIC: FUNC=Cosine, PRECISION=24 (высокая точность для FOC).
 * Конфигурируем и через HAL (hcordic), и через прямой MODIFY_REG CSR
 * для гарантии (на случай если HAL Defaults не такие как ожидаем).
 */
static void MX_CORDIC_Init(void)
{
  hcordic.Instance = CORDIC;
  if (HAL_CORDIC_Init(&hcordic) != HAL_OK) { Error_Handler(); }

  /* Явная конфигурация CORDIC для FOC:
   *   FUNC=0       -> Cosine (CORDIC_Cos напрямую, CORDIC_Sin через сдвиг на pi/2)
   *   PRECISION=24 -> высокая точность (по умолчанию 4 итераций недостаточно для токового контура)
   *   SCALE=0      -> корректно для cosine (выход в q31 [-1, 1])
   *   ARGSIZE=3    -> q31 angle в радианах [-pi, pi) (по умолчанию)
   *   RESSIZE=3    -> q31 output (по умолчанию)
   * Используем MODIFY_REG, чтобы не сбросить прочие поля CSR. */
  MODIFY_REG(CORDIC->CSR,
             CORDIC_CSR_FUNC_Msk | CORDIC_CSR_PRECISION_Msk,
             (0U  << CORDIC_CSR_FUNC_Pos)      |  /* FUNC = 0: Cosine */
             (24U << CORDIC_CSR_PRECISION_Pos));  /* PRECISION = 24 итераций */
}

/* ========== MX_TIM1_Init ==========
 * TIM1: master, center-aligned PWM, период 17000 (170 МГц / (2*17000) = 5 кГц).
 * RepetitionCounter=1 -> ISR каждые 2 PWM-периода (5 кГц при 10 кГц PWM).
 * AutoReloadPreload=ENABLE (защита от ARR glitch).
 * Break Input ENABLE, polarity LOW (активный ноль) - аппаратная защита power stage.
 * DeadTime=255 (~2.5 мкс при 170 МГц).
 */
static void MX_TIM1_Init(void)
{
  TIM_MasterConfigTypeDef sMasterConfig = {0};
  TIM_OC_InitTypeDef sConfigOC = {0};
  TIM_BreakDeadTimeConfigTypeDef sBreakDeadTimeConfig = {0};

  htim1.Instance = TIM1;
  htim1.Init.Prescaler = 0;
  htim1.Init.CounterMode = TIM_COUNTERMODE_CENTERALIGNED1;
  htim1.Init.Period = PWM_PERIOD;
  htim1.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
  htim1.Init.RepetitionCounter = 1;
  htim1.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE;
  if (HAL_TIM_PWM_Init(&htim1) != HAL_OK) { Error_Handler(); }

  sMasterConfig.MasterOutputTrigger = TIM_TRGO_UPDATE;
  sMasterConfig.MasterOutputTrigger2 = TIM_TRGO2_RESET;
  sMasterConfig.MasterSlaveMode = TIM_MASTERSLAVEMODE_ENABLE;
  if (HAL_TIMEx_MasterConfigSynchronization(&htim1, &sMasterConfig) != HAL_OK) { Error_Handler(); }

  sConfigOC.OCMode = TIM_OCMODE_PWM1;
  sConfigOC.Pulse = 0;
  sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
  sConfigOC.OCNPolarity = TIM_OCNPOLARITY_HIGH;
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
  sConfigOC.OCIdleState = TIM_OCIDLESTATE_RESET;
  sConfigOC.OCNIdleState = TIM_OCNIDLESTATE_RESET;
  if (HAL_TIM_PWM_ConfigChannel(&htim1, &sConfigOC, TIM_CHANNEL_1) != HAL_OK) { Error_Handler(); }
  __HAL_TIM_DISABLE_OCxPRELOAD(&htim1, TIM_CHANNEL_1);
  if (HAL_TIM_PWM_ConfigChannel(&htim1, &sConfigOC, TIM_CHANNEL_2) != HAL_OK) { Error_Handler(); }
  __HAL_TIM_DISABLE_OCxPRELOAD(&htim1, TIM_CHANNEL_2);
  if (HAL_TIM_PWM_ConfigChannel(&htim1, &sConfigOC, TIM_CHANNEL_3) != HAL_OK) { Error_Handler(); }
  __HAL_TIM_DISABLE_OCxPRELOAD(&htim1, TIM_CHANNEL_3);

  /* Аппаратная защита силового каскада (из Проекта 2):
   *   BreakState=ENABLE, BreakPolarity=LOW (активный ноль)
   *   DeadTime=255 (~2.5 мкс при 170 МГц)
   *   OSSR/OSSI=ENABLE - корректное управление off-state */
  sBreakDeadTimeConfig.OffStateRunMode = TIM_OSSR_ENABLE;
  sBreakDeadTimeConfig.OffStateIDLEMode = TIM_OSSI_ENABLE;
  sBreakDeadTimeConfig.LockLevel = TIM_LOCKLEVEL_OFF;
  sBreakDeadTimeConfig.DeadTime = PWM_DEADTIME;
#if PWM_BREAK_POLARITY_LOW
  sBreakDeadTimeConfig.BreakState = TIM_BREAK_ENABLE;
  sBreakDeadTimeConfig.BreakPolarity = TIM_BREAKPOLARITY_LOW;
#else
  sBreakDeadTimeConfig.BreakState = TIM_BREAK_DISABLE;
  sBreakDeadTimeConfig.BreakPolarity = TIM_BREAKPOLARITY_HIGH;
#endif
  sBreakDeadTimeConfig.BreakFilter = 0;
  sBreakDeadTimeConfig.BreakAFMode = TIM_BREAK_AFMODE_INPUT;
  sBreakDeadTimeConfig.Break2State = TIM_BREAK2_DISABLE;
  sBreakDeadTimeConfig.Break2Polarity = TIM_BREAK2POLARITY_HIGH;
  sBreakDeadTimeConfig.Break2Filter = 0;
  sBreakDeadTimeConfig.Break2AFMode = TIM_BREAK_AFMODE_INPUT;
  sBreakDeadTimeConfig.AutomaticOutput = TIM_AUTOMATICOUTPUT_DISABLE;
  if (HAL_TIMEx_ConfigBreakDeadTime(&htim1, &sBreakDeadTimeConfig) != HAL_OK) { Error_Handler(); }

  HAL_TIM_MspPostInit(&htim1);
}

/* ========== MX_TIM8_Init ==========
 * TIM8: slave, сброс по ITR0 (от TIM1). Считает синхронно с TIM1.
 * SLAVEMODE_RESET (вместо TRIGGER) - надёжнее для production.
 * Та же аппаратная защита, что у TIM1.
 */
static void MX_TIM8_Init(void)
{
  TIM_SlaveConfigTypeDef sSlaveConfig = {0};
  TIM_MasterConfigTypeDef sMasterConfig = {0};
  TIM_OC_InitTypeDef sConfigOC = {0};
  TIM_BreakDeadTimeConfigTypeDef sBreakDeadTimeConfig = {0};

  htim8.Instance = TIM8;
  htim8.Init.Prescaler = 0;
  htim8.Init.CounterMode = TIM_COUNTERMODE_CENTERALIGNED1;
  htim8.Init.Period = PWM_PERIOD;
  htim8.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
  htim8.Init.RepetitionCounter = 0;
  htim8.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE;
  if (HAL_TIM_Base_Init(&htim8) != HAL_OK) { Error_Handler(); }
  if (HAL_TIM_PWM_Init(&htim8) != HAL_OK) { Error_Handler(); }

  /* ИЗ Проекта 2: RESET вместо TRIGGER для надёжной синхронизации.
   * При TRIGGER таймер только стартует от master и далее считает сам -
   * при дрейфе PSC/ARR уйдёт из фазы. RESET переинициализирует фазу каждый период. */
  sSlaveConfig.SlaveMode = TIM_SLAVEMODE_RESET;
  sSlaveConfig.InputTrigger = TIM_TS_ITR0;
  if (HAL_TIM_SlaveConfigSynchro(&htim8, &sSlaveConfig) != HAL_OK) { Error_Handler(); }

  sMasterConfig.MasterOutputTrigger = TIM_TRGO_RESET;
  sMasterConfig.MasterOutputTrigger2 = TIM_TRGO2_RESET;
  sMasterConfig.MasterSlaveMode = TIM_MASTERSLAVEMODE_DISABLE;
  if (HAL_TIMEx_MasterConfigSynchronization(&htim8, &sMasterConfig) != HAL_OK) { Error_Handler(); }

  sConfigOC.OCMode = TIM_OCMODE_PWM1;
  sConfigOC.Pulse = 0;
  sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
  sConfigOC.OCNPolarity = TIM_OCNPOLARITY_HIGH;
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
  sConfigOC.OCIdleState = TIM_OCIDLESTATE_RESET;
  sConfigOC.OCNIdleState = TIM_OCNIDLESTATE_RESET;
  if (HAL_TIM_PWM_ConfigChannel(&htim8, &sConfigOC, TIM_CHANNEL_1) != HAL_OK) { Error_Handler(); }
  if (HAL_TIM_PWM_ConfigChannel(&htim8, &sConfigOC, TIM_CHANNEL_2) != HAL_OK) { Error_Handler(); }
  if (HAL_TIM_PWM_ConfigChannel(&htim8, &sConfigOC, TIM_CHANNEL_3) != HAL_OK) { Error_Handler(); }

  /* Та же аппаратная защита, что у TIM1 */
  sBreakDeadTimeConfig.OffStateRunMode = TIM_OSSR_ENABLE;
  sBreakDeadTimeConfig.OffStateIDLEMode = TIM_OSSI_ENABLE;
  sBreakDeadTimeConfig.LockLevel = TIM_LOCKLEVEL_OFF;
  sBreakDeadTimeConfig.DeadTime = PWM_DEADTIME;
#if PWM_BREAK_POLARITY_LOW
  sBreakDeadTimeConfig.BreakState = TIM_BREAK_ENABLE;
  sBreakDeadTimeConfig.BreakPolarity = TIM_BREAKPOLARITY_LOW;
#else
  sBreakDeadTimeConfig.BreakState = TIM_BREAK_DISABLE;
  sBreakDeadTimeConfig.BreakPolarity = TIM_BREAKPOLARITY_HIGH;
#endif
  sBreakDeadTimeConfig.BreakFilter = 0;
  sBreakDeadTimeConfig.BreakAFMode = TIM_BREAK_AFMODE_INPUT;
  sBreakDeadTimeConfig.Break2State = TIM_BREAK2_DISABLE;
  sBreakDeadTimeConfig.Break2Polarity = TIM_BREAK2POLARITY_HIGH;
  sBreakDeadTimeConfig.Break2Filter = 0;
  sBreakDeadTimeConfig.Break2AFMode = TIM_BREAK_AFMODE_INPUT;
  sBreakDeadTimeConfig.AutomaticOutput = TIM_AUTOMATICOUTPUT_DISABLE;
  if (HAL_TIMEx_ConfigBreakDeadTime(&htim8, &sBreakDeadTimeConfig) != HAL_OK) { Error_Handler(); }

  HAL_TIM_MspPostInit(&htim8);
}

/* ========== MX_USART2_UART_Init ========== */
static void MX_USART2_UART_Init(void)
{
  huart2.Instance = USART2;
  huart2.Init.BaudRate = 115200;
  huart2.Init.WordLength = UART_WORDLENGTH_8B;
  huart2.Init.StopBits = UART_STOPBITS_1;
  huart2.Init.Parity = UART_PARITY_NONE;
  huart2.Init.Mode = UART_MODE_TX_RX;
  huart2.Init.HwFlowCtl = UART_HWCONTROL_NONE;
  huart2.Init.OverSampling = UART_OVERSAMPLING_16;
  huart2.Init.OneBitSampling = UART_ONE_BIT_SAMPLE_DISABLE;
  huart2.Init.ClockPrescaler = UART_PRESCALER_DIV1;
  huart2.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
  if (HAL_UART_Init(&huart2) != HAL_OK) { Error_Handler(); }

  if (HAL_UARTEx_SetTxFifoThreshold(&huart2, UART_TXFIFO_THRESHOLD_1_4) != HAL_OK) { Error_Handler(); }
  if (HAL_UARTEx_SetRxFifoThreshold(&huart2, UART_RXFIFO_THRESHOLD_1_4) != HAL_OK) { Error_Handler(); }
  if (HAL_UARTEx_DisableFifoMode(&huart2) != HAL_OK) { Error_Handler(); }
}

/* ========== MX_GPIO_Init ========== */
static void MX_GPIO_Init(void)
{
  /* GPIO Ports Clock Enable */
  __HAL_RCC_GPIOC_CLK_ENABLE();
  __HAL_RCC_GPIOF_CLK_ENABLE();
  __HAL_RCC_GPIOA_CLK_ENABLE();
  __HAL_RCC_GPIOB_CLK_ENABLE();
}

/* ========== BSP callbacks ========== */
void BSP_PB_Callback(Button_TypeDef Button)
{
  if (Button == BUTTON_USER) {
    BspButtonState = BUTTON_PRESSED;
  }
}

/* ========== Error_Handler ========== */
void Error_Handler(void)
{
  __disable_irq();
  while (1) {}
}

#ifdef USE_FULL_ASSERT
void assert_failed(uint8_t *file, uint32_t line)
{
  /* Пользователь может добавить вывод имени файла и номера строки:
     printf("Wrong parameters value: file %s on line %d\r\n", file, line) */
}
#endif /* USE_FULL_ASSERT */
