...
| ai-bench (стенд) | ПК-1 | ai-bench/ct-fix | ТЗ CT-канал/калибровка: применён e73b8f0 + честный FAIL в CLI `c`; стенд (no-HV): offset_i1=2039, фантомы 26А→12-38мА, FAULT=0, FOC rc=-2 map, c→FAIL rc=-1, ADC stable (I1 spread 4, I2 spread 8), ENC err=0 period=898, PWM off, vdc=150 OK, нет энергии | src/adc.c/h, src/cli.c | готово к приёмке (CI на ubuntu) |
