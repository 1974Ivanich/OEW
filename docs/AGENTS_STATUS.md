...
| ai-bench (стенд) | ПК-1 | ai-bench/ct-fix | ТЗ CT-канал/калибровка: e73b8f0 + честный FAIL; no-HV полностью PASS: offset_i1=2039, фантомы 26А→12-38мА, FAULT=0, FOC rc=-2, ADC stable, ENC err=0, soak 60с, CLI robustness, UART stress 50/50, config (dt/pp/vfk/s/i) применяются без energise, PWM off | src/adc.c/h, src/cli.c | готово к приёмке (CI на ubuntu) |
