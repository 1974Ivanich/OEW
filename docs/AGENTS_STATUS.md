...
| ai4 (Manus) | ПК-1 | ai4/bench-test2-g0-check | ТЗ Hard Gate G0: fail-closed offline validator approval, diagnostic-build manifest/log и source→firmware SHA evidence для physical no-HV Test №2; не обращается к hardware | TZ_BENCH_TEST2_G0_CHECK.md, tools/bench_test2_g0_check.py/.md, tests/test_bench_test2_g0_check.py, README.md, docs/AGENTS_STATUS.md | make + make test PASS (96 pytest); flash намеренно не запускался; готово к CI-приёмке, physical HIL остаётся NO-GO до explicit safety-owner approval |
