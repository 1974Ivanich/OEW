...
| ai-bench (стенд) | ПК-1 | ai-bench/ct-fix | ТЗ CT-канал/калибровка: применён e73b8f0 (retain shunt offsets) + честный FAIL в CLI `c`; стенд: offset_i1=2038, фантомы 26А→0-38мА, FAULT=0, FOC rc=-2 map, c→FAIL rc=-1 (Ires=0), ENC err=0 | src/adc.c/h, src/cli.c | готово к приёмке (CI на ubuntu) |
