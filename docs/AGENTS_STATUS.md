...
| ai-bench (стенд, ПК-3) | ПК-3 | ai-bench/map-capture-profile | ТЗ board-qualified профиль MapCapture: energise-кампания заблокирована fail-closed (заглушки профилей); этап A — данные стенда (HV-допуск), этап B — реализация allow-list + тесты | src/map_capture_profiles.c/h, tests/map_capture_profiles_test.c, main.c | ТЗ оформлено, ожидает допуска/данных |
