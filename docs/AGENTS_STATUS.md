...
| ai4 (Manus) | ПК-1 | ai4/bench-test2-preflight-cli | ТЗ CLI pre-flight Test №2: fail-closed G0/artifact validation, operator confirmations и разрешённые UART observations без arm/run/flash | TZ_BENCH_TEST2_PREFLIGHT_CLI.md, tools/bench_test2_preflight.py/.md, tests/test_bench_test2_preflight.py, README.md, docs/AGENTS_STATUS.md | make + make test PASS (129 pytest); flash намеренно не запускался; готово к CI-приёмке, не обращалось к hardware |
