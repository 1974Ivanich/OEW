...
> **Примечание от 2026-09-19 (добавлено после факта; исторический текст выше не изменён).**
> Упомянутая «локальная правка `scripts/cubemx_check_script.txt`» была **симптомом дефекта
> механизма**: `scripts/cubemx_check.py` писал генерируемый скрипт headless-прогона CubeMX в
> tracked-путь `scripts/`, из-за чего любой прогон проверки (в том числе из pre-push hook) делал
> рабочее дерево грязным, причём с абсолютными путями того ПК, где шла проверка. Симптом
> наблюдался на двух ПК: на ПК-3 как незакоммиченная локальная правка, на ПК-2 как `M` в
> `git status` после каждого push. Дефект устранён в ветке `ai2/cubemx-hygiene` (`ff9ede88`):
> артефакт перенесён в `logs/cubemx_check_script.txt` (untracked + `.gitignore`), запись идёт
> ровно CRLF, инвариант «проверка не меняет tracked-файлы» проверяется
> `tests/test_cubemx_hygiene.py` (включая реальный прогон CubeMX). Локальная правка этого файла
> больше не требуется, сам файл из репозитория удалён.
