...
- [ ] Logic analyzer single-shot includes at least 2 ms before and 5 ms after
  PB6 falling edge, with a sample rate sufficient to resolve short SD pulses.
...
5. [ ] Enable supply; confirm 9–11 V at the inverter with DMM and firmware.
6. [ ] Confirm `FAULT=0`, PWM off, SD1/SD2 high.
...
8. [ ] Disable supply immediately after post-burst checks. Apply LOTO and verify
   DC-link < 1 V.
9. [ ] Save all UART, scope, logic, supply and LED observations before deciding
   on escalation.
...
## 5. Optional escalation — 15 V, then 20 V maximum
...
- Step B: repeat section 4 once at 15 V (acceptable measured range 14–16 V).
- Step C: repeat section 4 once at 20 V (acceptable measured range 19–21 V).
...
## 7. Mandatory shutdown and restore
