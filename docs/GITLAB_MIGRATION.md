...
Статус: **подготовка** (ветка `ai-hermes/gitlab-migration-prep`).
Решение: GitLab.com (облако, shared runners), мигрирует **только OEW**,
GitHub остаётся **read-only архивом** (история/ветки не удаляются).

Проверено: gitlab.com доступен с ПК-1 (301 на `/`, API отвечает);
сетевых блокировок нет (в отличие от st.com/nxp.com).
