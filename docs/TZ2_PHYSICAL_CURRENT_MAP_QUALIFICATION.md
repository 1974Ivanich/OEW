...
## 15. Предлагаемый независимый reference: 3 × INA240A1 + 4-канальный осциллограф

Для следующей физической кампании предлагается заменить два ACS712-20A (U/V) на три независимых измерительных канала:

```
U phase -> RSHUNT_U 30 mOhm (Kelvin) -> INA240A1 (20 V/V) -> scope CH1
V phase -> RSHUNT_V 30 mOhm (Kelvin) -> INA240A1 (20 V/V) -> scope CH2
W phase -> RSHUNT_W 30 mOhm (Kelvin) -> INA240A1 (20 V/V) -> scope CH3
frame marker -> dedicated GPIO -> scope CH4
```

PA6 сохраняется как существующий `ADC2_IN3/raw_ct` и не используется одновременно как цифровой frame marker. Для CH4 должен быть выбран отдельный GPIO; PA4/PA5 в текущем проекте подготовлены как bench-only logic markers, но их фактическое per-frame переключение требует отдельной проверки/реализации.

### 15.1 Аналоговый канал

Для каждого INA240A1:

- питание `VS = +5 V`;
- `REF1 = GND`;
- `REF2 = +5 V`;
- `IN+` — Kelvin-съём со стороны inverter относительно шунта;
- `IN-` — Kelvin-съём со стороны motor относительно шунта;
- `OUT` — соответствующий вход осциллографа;
- локальная развязка питания не менее 100 nF, предпочтительно дополнительно 1 uF.

При `Rshunt = 30 mOhm` и gain 20 V/V:

```
Vout = Vref + I * 0.03 * 20
Vout = 2.5 V + 0.6 V/A * I
```

Таким образом, при двунаправленном токе ±3 A ожидаемый выходной диапазон составляет примерно 0.7…4.3 V. При 3 A установившаяся мощность одного 30-mOhm шунта:

```
P = I^2 R = 3^2 * 0.03 = 0.27 W
```

Для стенда рекомендуется Kelvin-шунт 30 mOhm с номинальной мощностью порядка 1 W или с эквивалентным подтверждённым импульсным/температурным запасом.

Фактический допустимый фазный ток не следует выводить только из ограничения лабораторного БП 60 V / 3 A: фазный ток и DC-link ток не обязаны быть равны при PWM/реактивной нагрузке. Перед включением требуется проверить реальный токовый диапазон и common-mode условия конкретного шунта/монтажа.

### 15.2 Четыре канала осциллографа
...
Три фазных тока становятся независимыми физическими reference channels. KCL:

```
I_U + I_V + I_W ~= 0
```

используется как самостоятельный контроль измерительной цепи, а не для получения W.

### 15.3 Требование к temporal association

Один PB6-фронт, существующий в текущем V/f/map-capture пути, является session marker и не является frame marker. Поэтому deadtime resolution 0.125 us @ 8 MHz нельзя переносить в `DT_UNCERTAINTY_US` phase association.

Для настоящей association необходимо физически наблюдаемый per-frame marker на CH4, однозначно связанный с ADC/PWM sampling event. До доказательства такой связи:

```
DT_METHOD       = unresolved
DT_WINDOW_US_*  = unresolved
DT_UNCERTAINTY_US = unresolved
```

### 15.4 Qualification consequences

После перехода на три независимых INA240 channels reference declaration должна отражать фактическую схему:

```
REFERENCE_PHASES = U,V,W independently measured
W is NOT derived by KCL
```

Калибровка должна иметь независимые параметры U/V/W (нулевой уровень и scale), полученные для соответствующих физических каналов. Старое правило `W derived by KCL` относится только к текущему ACS712 U/V варианту и после аппаратной замены не применяется.

Negative association test остаётся temporal shift/rotation scope rows. `U<->V` swap остаётся только диагностикой инвариантности solver/pipeline к именованию фаз и не считается physical-association rejection test.

### 15.5 Safety boundary

Замена reference instrumentation сама по себе не меняет firmware admission, protection, map selector или PWM gates. До аппаратного монтажа необходимо отдельно проверить datasheet/common-mode limits конкретного INA240A1, конструкцию Kelvin-шунта, изоляцию/землю осциллографа и безопасный VBUS согласно действующему G0.

Это предложение не является разрешением подавать 60 V. Значение `60 V / 3 A` — характеристика доступного БП; допустимый VBUS для конкретной кампании определяется действующим G0 и аппаратными ограничениями стенда.
