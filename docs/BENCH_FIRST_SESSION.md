...
- [ ] Период 700–1500 мкс (номинал ~1087 мкс/920 Гц; на стенде 897 мкс — RC-генератор AS5048A плавает, err=0 = валидно) — если `err=1` (TIMEOUT) или `err=2` (BAD_PERIOD) — проверить провод/подтяжку
