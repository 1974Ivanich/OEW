...
**Назначение:** минимальная физическая проверка измерительной цепи и ADC на полном стенде без DC-link. Этот checklist не является разрешением на Test №3, MapCapture, FOC, V/f, autotune или Stage A.
...
В `report.md` укажите дату/время UTC, оператора, source SHA, board/MCU identity, firmware/verify result, два DMM-показания DC-link, aux voltage, 10 строк ADC-наблюдений и итог `PASS`/`FAIL`/`BLOCKED`. Полный UART-лог не редактировать и не сокращать.
...
`Test №2 PASS` подтверждает только физический ADC/measurement-chain baseline при no-HV. Он не разрешает Test №3, `mapcap run`, RealBoardProfile, automatic characterization, DC-link, Stage A, FOC, V/f или autotune.
