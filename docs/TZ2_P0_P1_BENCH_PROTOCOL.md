...
### 2.1 Zero-noise measurement (обязательно до окончательного waiver по масштабу)

Две несогласованные цифры шума канала (`~11 мВ pp` в `tools/map_scope_ingest.md` и `~100 мВ pp`
в `docs/STEP_A_ACCEPTANCE.md`) должны быть сведены к одной измеренной величине. Условия замера:

```text
датчики ACS712 питаются 5 В (Vcc измерен и записан)
ток в цепи = 0 (мотор обесточен, PWM закрыт: CCER=0, MOE=0)
осциллограф: AC-связь, 20 мВ/дел, полоса зафиксирована и записана
окно записи: достаточно длинное (≥ 100 периодов ШИМ), сырой экспорт сохранён
```

Вернуть: `Vpp`, `Vrms`, полосу, длительность окна, конфигурацию скопа/проб, sha сырого файла.
Правило: **брать консервативную цифру до замера**; после замера — оставить измеренную и пометить
старую оценку как неподтверждённую. Если `Vpp` подтвердится на уровне ~100 мВ, waiver по масштабу
на этой конфигурации становится безусловным (SNR ≤ 1 при токах ≤ 1 А).
...
### 7.1 Шаблон вердикта кампании P0/P1 на 60 В (waiver по масштабу)

```text
ADC chain              QUALIFIED
PWM <-> ADC timing     QUALIFIED
PB6 trigger            QUALIFIED
phase mapping          QUALIFIED
capture integrity      QUALIFIED
safety invariants      QUALIFIED

current zero           OBSERVED
current amplitude      OBSERVED ONLY
current scale          NOT DEMONSTRATED
G3                     BLOCKED
```

Формулировка причины (обязательна дословно в отчёте):

> **The independent ACS712 reference is insufficiently resolved at the measured current range.
> Therefore the session does not qualify current-scale accuracy.**

Условие применения waiver: выполнен §2.1 (замер нулевого шума) и зафиксировано, что предпосылка
«на 60 В будет несколько ампер» **опровергнута измерением** (0.13…0.91 А на штатных векторах профиля),
то есть это дефект предпосылки, а не недобор тока оператором. Нормальный независимый токовый референс
(фазный шунт + инструментальный усилитель) — отдельное ТЗ (`docs/TZ_REF_01_...`, статус DRAFT).
