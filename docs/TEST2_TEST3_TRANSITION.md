...
| [TEST2_TEST3_MAP_ROUTE.md](TEST2_TEST3_MAP_ROUTE.md) | Упрощённый маршрут: один Test №2 baseline approval, один объединённый Test №3 no-HV plan и последующая physical characterization без дублирующих mini-tests. |
| [tools/test2_test3_route_check.md](../tools/test2_test3_route_check.md) | Offline fail-closed проверка готовности route evidence/12-context plan; не заменяет G0 или Stage A approval. |
...
Until that package is accepted, diagnostic flash, `mcarm`, `mapcap run`, RealBoardProfile and automatic characterization remain **NO-GO**. The compact route checker documents Test №2 approval and a unified Test №3 plan only; it deliberately does not bypass this migration/G0 boundary or produce `REAL_BOARD_CAPTURE_VALID`.
