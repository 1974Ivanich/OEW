...
static uint32_t crc32_update(uint32_t crc, uint8_t byte)
{
    uint8_t bit;
    crc ^= byte;
    for (bit = 0u; bit < 8u; ++bit) crc = (crc & 1u) ? ((crc >> 1u) ^ 0xEDB88320u) : (crc >> 1u);
    return crc;
}
uint32_t CurrentMap_CalculateCrc32(const OewCurrentMap *map)
{
    const uint8_t *bytes; size_t i; const size_t crc_offset = offsetof(OewCurrentMap, crc32);
...
static bool region_is_sane(const OewPwmRegion *region)
{
    ...
    if (region->reserved == 1u && (region->geometry_mod_min_q15 <= 0 ||
                            ...
                            int16_t mu, int16_t mv, int16_t mw)
{
    ...
    if (region->reserved == 1u) {
        ...
        return geometry_sector_for_vector(mu, mv, mw) == sector &&
               mod >= region->geometry_mod_min_q15 &&
               (window == 0u ? mod < region->geometry_mod_max_q15 : mod <= region->geometry_mod_max_q15);
    }
...
static bool geometry_regions_pairwise_sane(const OewCurrentMap *map)
{
    ...
    for (sector = 0u; sector < OEW_CURRENT_MAP_SECTOR_COUNT; ++sector) {
        ...
        if (!region_is_sane(w0) || !region_is_sane(w1) || w0->reserved != 1u || w1->reserved != 1u) return false;
...
{
    uint8_t sector, window, other_sector, other_window;
    for (sector = 0u; sector < OEW_CURRENT_MAP_SECTOR_COUNT; ++sector) {
        for (window = 0u; window < OEW_CURRENT_MAP_WINDOW_COUNT; ++window) {
            const OewPwmRegion *current = &map->region[sector][window];
            if (!region_is_sane(current) || current->reserved != 0u || !map->recon[sector][window].valid) return false;
...
void CurrentMap_Reset(void) { memset(&g_map, 0, sizeof(g_map)); g_ready = 0u; CurrentRecon_Reset(); }
bool CurrentMap_LoadMeasured(const OewCurrentMap *map, const OewMapIdentity *active_identity)
{
    ...
    if (map->startup_sector >= OEW_CURRENT_MAP_SECTOR_COUNT || map->startup_window >= OEW_CURRENT_MAP_WINDOW_COUNT ||
    ...
    memcpy(&g_map, map, sizeof(g_map)); g_ready = 1u; return true;
}
bool CurrentMap_IsReady(void) { return g_ready != 0u && CurrentRecon_IsReady(); }
uint16_t CurrentMap_GetStartupHoldCycles(void) { return CurrentMap_IsReady() ? g_map.startup_hold_cycles : 0u; }
bool CurrentMap_SelectInitialStartupContext(PwmSampleContext *context, int16_t *mu, int16_t *mv, int16_t *mw)
{
    ...
    if (!region_contains(region, g_map.startup_sector, g_map.startup_window, g_map.startup_mu, g_map.startup_mv, g_map.startup_mw)) return false;
    *mu = g_map.startup_mu; *mv = g_map.startup_mv; *mw = g_map.startup_mw;
    context->sector = g_map.startup_sector; context->window = g_map.startup_window; context->valid = true; return true;
}
bool CurrentMap_SelectNextContext(int16_t mu, int16_t mv, int16_t mw, PwmSampleContext *context)
{
    uint8_t sector, window, matches = 0u;
    ...
    for (sector = 0u; sector < OEW_CURRENT_MAP_SECTOR_COUNT; ++sector)
        for (window = 0u; window < OEW_CURRENT_MAP_WINDOW_COUNT; ++window)
            if (region_contains(&g_map.region[sector][window], sector, window, mu, mv, mw)) {
                selected.sector = sector; selected.window = window; selected.valid = true; ++matches;
            }
    if (matches != 1u) return false;
    *context = selected; return true;
}
