                                   ...
                                   int16_t *min_mod, int16_t *max_mod)
{
    ...
    if (window == 0u) {
        lo = q->geometry_window0_min_mod_q15 ? q->geometry_window0_min_mod_q15 : GEOMETRY_DEFAULT_W0_MIN;
        hi = q->geometry_window0_max_mod_q15 ? q->geometry_window0_max_mod_q15 : GEOMETRY_DEFAULT_W0_MAX;
    } else {
        lo = q->geometry_window1_min_mod_q15 ? q->geometry_window1_min_mod_q15 : GEOMETRY_DEFAULT_W1_MIN;
        hi = q->geometry_window1_max_mod_q15 ? q->geometry_window1_max_mod_q15 : GEOMETRY_DEFAULT_W1_MAX;
    }
    if (lo <= 0 || hi <= lo) return false;
    *min_mod = lo; *max_mod = hi;
    return true;
}
                                  ...
                                  bool geometry)
{
    ...
    for (i = 0u; i < cell_count; ++i) {
        if (cells[i].status == 1u) {
            ++valid;
            mu_min = min_i16(mu_min, cells[i].mu); mv_min = min_i16(mv_min, cells[i].mv);
            mw_min = min_i16(mw_min, cells[i].mw); mu_max = max_i16(mu_max, cells[i].mu);
            mv_max = max_i16(mv_max, cells[i].mv); mw_max = max_i16(mw_max, cells[i].mw);
    ...
    if (geometry) {
        ...
        if (mu_max - mu_min < 2 * q->guard_q15 || mv_max - mv_min < 2 * q->guard_q15 ||
            mw_max - mw_min < 2 * q->guard_q15) return MAP_CERT_DEGENERATE;
        memset(out, 0, sizeof(*out));
        ...
        out->geometry_mod_min_q15 = 0; out->geometry_mod_max_q15 = 0; out->reserved = 0u;
    }
    out->min_margin_ticks = min_margin; out->valid = 1u;
    for (i = 0u; i < cell_count; ++i) {
        bool in = geometry ? inside_geometry(out, sector, window, &cells[i]) : inside_statistical(out, &cells[i]);
