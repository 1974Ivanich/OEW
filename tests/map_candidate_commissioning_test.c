...
static MapReferenceManifest manifest_make(OewMapIdentity *identity)
{
    ...
    memset(&m, 0, sizeof(m));
    identity->board_revision = 7u; identity->pwm_frequency_hz = 5000u;
    identity->timer_arr = 1000u; identity->adc_trigger_id = 0x4F455731u;
    identity->adc_clock_hz = 42500000u;
    identity->adc_sample_cycles_x2 = 1281u;
    identity->adc_resolution = 0u;
    identity->deadtime_ticks = 0x0Fu;
    ...
    m.tool_build_id = 0x20260820u; m.record_count = MAP_CANDIDATE_REQUIRED_ROWS;
...
static MapCandidateQualification qualification_make(void)
{
    ...
    q.startup_sector = 0u; q.startup_window = 0u;
    q.startup_hold_cycles = 4u;
    q.startup_mu = 0; q.startup_mv = 0; q.startup_mw = 0;
    return q;
}
                        ...
                        OewPwmRegion regions[OEW_CURRENT_MAP_SECTOR_COUNT][OEW_CURRENT_MAP_WINDOW_COUNT])
{
    ...
    for (s = 0u; s < OEW_CURRENT_MAP_SECTOR_COUNT; ++s) {
        for (w = 0u; w < OEW_CURRENT_MAP_WINDOW_COUNT; ++w) {
            recon[s][w].valid = true; recon[s][w].phase_a = 0u; recon[s][w].phase_b = 1u;
            recon[s][w].m00 = 1000; recon[s][w].m01 = 0;
            recon[s][w].m10 = 0; recon[s][w].m11 = 1000;
            regions[s][w].mu_min = -1000; regions[s][w].mu_max = 1000;
            regions[s][w].mv_min = -1000; regions[s][w].mv_max = 1000;
            regions[s][w].mw_min = -1000; regions[s][w].mw_max = 1000;
            regions[s][w].min_margin_ticks = 5u; regions[s][w].valid = 1u;
        }
    }
}
...
{
    assert(map != 0 && identity != 0); ++g.loads; return true;
}
...
static MapCommissioningOps ops_make(void)
{
    ...
    ops.capture_active = capture_active; ops.foc_running = false_fn;
    ops.vfc_running = false_fn; ops.autotune_active = false_fn;
    ops.pwm_enabled = false_fn; ops.adc_injected_armed = false_fn;
    ops.protect_fault = fault_fn; ops.get_identity = get_identity;
    ops.load_measured = load_map; ops.map_ready = map_ready;
    return ops;
}
...
static void test_candidate_and_crc_rejection(void)
{
    ...
    g.identity = q.identity; g.ready = true; g.loads = 0u; g.active = false; g.fault = false;
...
static void test_active_control_and_identity_rejection(void)
{
    ...
    OewCurrentMap map = build_map(&q);
    MapCommissioningOps ops = ops_make();
    g.identity = q.identity; g.ready = true; g.loads = 0u; g.active = true; g.fault = false;
    assert(!MapCommissioning_LoadMeasured(&map, &q.manifest, &ops));
    assert(g.loads == 0u);
    g.active = false; g.identity.board_revision++;
    assert(!MapCommissioning_LoadMeasured(&map, &q.manifest, &ops));
    assert(g.loads == 0u);
}
...
static void test_incomplete_candidate_rejected(void)
{
    ...
    OewPwmRegion regions[OEW_CURRENT_MAP_SECTOR_COUNT][OEW_CURRENT_MAP_WINDOW_COUNT];
    OewCurrentMap map;
    fill_tables(recon, regions); recon[5][1].valid = false;
