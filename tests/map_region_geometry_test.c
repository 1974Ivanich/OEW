...
static const int16_t reps[6][3] = {
    { 8000,     0, -8000 }, { 8000, -8000,     0 },
    {    0,  8000, -8000 }, {-8000,  8000,     0 },
    {    0, -8000,  8000 }, {-8000,     0,  8000 }
};
static const int16_t high_reps[6][3] = {
    {12000,     0, -12000 }, {12000,-12000,     0 },
    {    0, 12000,-12000 }, {-12000,12000,     0 },
    {    0,-12000, 12000 }, {-12000,    0, 12000 }
};
...
static void test_geometry_12_regions(void)
{
    ...
    for (s = 0u; s < 6u; ++s) {
        for (w = 0u; w < 2u; ++w) {
            ...
            assert(MapRegionCertifyForSectorWindow(cells, 4u, s, w, &q,
                                                    &out, &report) == MAP_CERT_OK);
...
static void test_boundary_ownership(void)
{
    ...
    /* U=V and V=W boundaries use the U<V<W tie-break convention. */
    cells_make(cells, reps[0]);
...
static void fill_geometry_map(OewCurrentMap *map, OewMapIdentity *id)
{
    ...
    map->adc_config_signature = id->adc_config_signature;
    map->current_calibration_signature = id->current_calibration_signature;
    ...
    for (s = 0u; s < 6u; ++s) for (w = 0u; w < 2u; ++w) {
        ...
        CurrentReconEntry *c = &map->recon[s][w];
        r->mu_min = INT16_MIN; r->mu_max = INT16_MAX;
        r->mv_min = INT16_MIN; r->mv_max = INT16_MAX;
        r->mw_min = INT16_MIN; r->mw_max = INT16_MAX;
        r->min_margin_ticks = 110u;
        r->geometry_mod_min_q15 = w == 0u ? 6000 : 10000;
        r->geometry_mod_max_q15 = w == 0u ? 10000 : 14000;
        r->valid = 1u; r->reserved = 1u;
        c->valid = true; c->phase_a = 0u; c->phase_b = 1u;
        c->m00 = 1000; c->m01 = 0; c->m10 = 0; c->m11 = 1000;
    }
...
static void test_geometry_admission_overlap(void)
{
    OewCurrentMap map;
    OewMapIdentity id;
    PwmSampleContext ctx = { 9u, 9u, false };
    fill_geometry_map(&map, &id);
    CurrentMap_Reset();
    assert(CurrentMap_LoadMeasured(&map, &id));
    assert(CurrentMap_SelectNextContext(8000, 0, -8000, &ctx));
    assert(ctx.sector == 0u && ctx.window == 0u);
    assert(CurrentMap_SelectNextContext(12000, 0, -12000, &ctx));
    assert(ctx.sector == 0u && ctx.window == 1u);
    assert(CurrentMap_SelectNextContext(10000, -5000, -5000, &ctx));
    assert(ctx.sector == 0u && ctx.window == 1u);
    assert(!CurrentMap_SelectNextContext(5000, 0, -5000, &ctx));

    map.region[0][1].geometry_mod_min_q15 = 9000;
    map.crc32 = CurrentMap_CalculateCrc32(&map);
    assert(!CurrentMap_LoadMeasured(&map, &id));
...
int main(void)
{
    test_geometry_12_regions();
    test_certified_outside_and_status();
    test_boundary_ownership();
    test_geometry_admission_overlap();
    test_statistical_fallback();
    puts("map_region_geometry_test: PASS");
    return 0;
}
