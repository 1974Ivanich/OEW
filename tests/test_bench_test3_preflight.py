...
SCRIPT = REPO / "tools" / "bench_test3_preflight.py"
SPEC = importlib.util.spec_from_file_location("bench_test3_preflight", SCRIPT)
...
def write_campaign(tmp_path: Path) -> Path:
    campaign = tmp_path / "test3_nohv_20260825T120000Z"
    campaign.mkdir()
    ...
    write_json(campaign / "g0_check_summary.json", {"gate": "HIL_TEST3_G0", "verdict": "PASS"})
    return campaign
...
def test_g0_failure_blocks_real_uart_before_open(tmp_path: Path,
                                                  monkeypatch: pytest.MonkeyPatch) -> None:
    campaign = write_campaign(tmp_path)
    write_json(campaign / "g0_check_summary.json", {"gate": "HIL_TEST3_G0", "verdict": "FAIL"})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda campaign: write_json(campaign / "g0" / "g0_check_summary.json", {"gate": "HIL_TEST3_G0", "verdict": "PASS"}), "g0.summary.path"),
    ],
)
