...
3. **Регрессии (hosted)**: пакетный тест с реальными bounded-полями — `snprintf` length < буфера,
   терминатор на месте, бюджет 50 Гц задокументирован; snapshot совместимости legacy `@VFLOG`.
...
новые hosted-регрессии PASS; `git diff --check`; CI ветки success; scope — только `main.c`/`src/uart.c`/тесты.
