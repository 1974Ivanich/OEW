...
CAPTURE_PATH = TOOLS_DIR / "bench_test3_capture.py"
_CAPTURE_SPEC = importlib.util.spec_from_file_location("_test3_capture_contract", CAPTURE_PATH)
...
def validate_g0(campaign: Path, recorder: Recorder) -> dict[str, str]:
    ...
    if manifest is not None:
        recorder.add("g0.manifest.target", "OFFLINE", "physical-nohv-diagnostic-test3", manifest.get("target"),
                     manifest.get("target") == "physical-nohv-diagnostic-test3",
                     "Manifest target must remain no-HV diagnostic.")
        recorder.add("g0.manifest.test", "OFFLINE", "MAPCAP_TEST3", manifest.get("test"),
                     manifest.get("test") == "MAPCAP_TEST3", "Manifest test must identify Test №2.")
